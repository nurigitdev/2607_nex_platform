...
SCHEMA_VERSION = "ae_artifact_retention_purge_postgres_smoke.v1"
SMOKE_ENV = "NEX_AE_ARTIFACT_RETENTION_PURGE_POSTGRES_SMOKE"
SMOKE_PROFILE_ENV = "NEX_AE_ARTIFACT_RETENTION_PURGE_POSTGRES_SMOKE_PROFILE"
...
def run_ae_artifact_retention_purge_postgres_smoke(
    ...
    try:
        ...
        execution = _execute_ae_artifact_retention_purge_smoke(
            database_url=database_url,
...
def _execute_ae_artifact_retention_purge_smoke(
    *,
    database_url: str,
    database_env: str,
) -> dict[str, Any]:
    ...
    tenant_id = f"tenant-artifact-purge-{suffix}"
    workspace_id = f"workspace-artifact-purge-{suffix}"
    owner_user_id = f"owner-artifact-purge-{suffix}"
    artifact_ids: list[str] = []
    handoff_ids: list[str] = []
    engine = build_engine(database_url)
    try:
        session_factory = build_session_factory(engine)
        with tempfile.TemporaryDirectory(prefix="nex-ae-artifact-purge-smoke-") as storage_dir:
            ...
            with artifact_pg._temporary_env(
                ...
                dry_run = _post_purge(
                    ...
                    idempotency_key=f"retention-purge-dry-run-{suffix}",
                )
                after_dry_run = _db_observations(
                    engine,
                    tenant_id=tenant_id,
                    workspace_id=workspace_id,
                    owner_user_id=owner_user_id,
                ...
                blocked = _post_purge(
                    client,
                    headers,
                    payload={**purge_payload, "dry_run": False},
                    idempotency_key=f"retention-purge-blocked-{suffix}",
                )
                after_blocked = _db_observations(
                    engine,
                    tenant_id=tenant_id,
                    workspace_id=workspace_id,
                    owner_user_id=owner_user_id,
                ...
                after_execute = _db_observations(
                    engine,
                    tenant_id=tenant_id,
                    workspace_id=workspace_id,
                    owner_user_id=owner_user_id,
                ...
                old_readback = _get_artifact(client, headers, old_deleted["artifact_id"])
                recent_readback = _get_artifact(
                    client,
                    headers,
                    recent_deleted["artifact_id"],
                )
                execute_body = execute["body"]
                checks = {
                    "initial_candidate_rows": before["candidate_rows"] == 1,
                    "initial_deleted_rows": before["deleted_rows"] == 2,
                    "initial_file_rows": before["file_rows"] >= 4,
                    "initial_link_rows": before["link_rows"] >= 8,
                    ...
                    "dry_run_selected_one": dry_run["body"].get("selected_count") == 1,
                    "dry_run_deleted_zero": not any(
                        dry_run["body"].get("deleted_counts", {}).values()
                    ),
                    "dry_run_rows_retained": after_dry_run == before,
                    "blocked_route_ok": blocked["status_code"] == 200,
                    "blocked_execute": blocked["body"].get("execution_status")
                    == "BLOCKED",
                    ...
                    == 1,
                    "execute_deleted_files": execute_body.get(
                        "deleted_counts",
                        {},
                    ).get("files")
                    == 2,
                    "execute_deleted_links": execute_body.get(
                        "deleted_counts",
                        {},
                    ).get("links")
                    == 4,
                    "execute_deleted_storage_files": execute_body.get(
                        "deleted_counts",
                        {},
                    ).get("storage_files")
                    == 2,
                    "db_old_artifact_deleted": after_execute["artifact_rows"] == 1,
                    "db_recent_deleted_retained": after_execute["deleted_rows"] == 1,
                    "db_candidate_rows_zero": after_execute["candidate_rows"] == 0,
                    "db_file_rows_removed": after_execute["file_rows"] == 2,
                    "db_link_rows_removed": after_execute["link_rows"] == 4,
                    "db_handoff_rows_retained": after_execute["handoff_rows"] == 2,
                    "storage_files_removed": materialized_after == 2,
                    "old_readback_404": old_readback["status_code"] == 404,
                    "recent_readback_retained": recent_readback["status_code"] == 200
                    and recent_readback["body"].get("artifact_status") == "DELETED",
                ...
                if failed_checks:
                    raise RuntimeError(
                        "AE artifact retention purge PostgreSQL smoke checks "
...
def _db_observations(
    ...
    *,
    ...
    observations = candidate_pg._db_observations(
        engine,
        tenant_id=tenant_id,
        workspace_id=workspace_id,
        owner_user_id=owner_user_id,
    ...
    with engine.connect() as connection:
        observations["handoff_rows"] = candidate_pg._scalar_count(
            connection,
            """
            SELECT count(*)
    ...
    for key, value in _sensitive_env_values(environ):
        if value in serialized_evidence:
            if key.endswith(":password"):
                raise ValueError(
                    "AE artifact retention purge smoke contains a database password."
                )
            raise ValueError(
                f"AE artifact retention purge smoke contains raw {key}."
            )
    if "/data/nex-platform" in serialized_evidence:
        raise ValueError(
            "AE artifact retention purge smoke contains a local data path."
        )
...
def summary_line(evidence: dict[str, Any]) -> str:
    ...
    if evidence["status"] == "PASS":
        return (
            "ae_artifact_retention_purge_postgres_smoke=pass "
            ...
            f"deleted_files={evidence['retention']['deleted_counts']['files']} "
            f"live_db={str(evidence['live_db']).lower()} "
            f"cleanup_artifacts={evidence['cleanup']['artifacts']} "
            f"cleanup_handoffs={evidence['cleanup']['handoffs']}"
        )
    return (
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run optional AE artifact retention purge PostgreSQL smoke."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    evidence = run_ae_artifact_retention_purge_postgres_smoke()
