...
SCHEMA_VERSION = (
    "ae_ag_artifact_retention_scheduler_daemon_supervisor_read_model_postgres_smoke.v1"
)
SMOKE_ENV = (
    "NEX_AE_AG_ARTIFACT_RETENTION_SCHEDULER_DAEMON_SUPERVISOR_READ_MODEL_POSTGRES_SMOKE"
)
SMOKE_PROFILE_ENV = (
    "NEX_AE_AG_ARTIFACT_RETENTION_SCHEDULER_DAEMON_SUPERVISOR_READ_MODEL_POSTGRES_SMOKE_PROFILE"
)
SERVICE_ID = supervisor_pg.SERVICE_ID
AG_SERVICE_ID = "nex-ag"
DEFAULT_PROFILE = supervisor_pg.DEFAULT_PROFILE


class AeTestClientDaemonSupervisorReadModelClient:
    ...
    def __init__(self, client: TestClient, headers: Mapping[str, str]) -> None:
        ...
        self.supervisor_collection_statuses: list[int] = []
        self.supervisor_detail_statuses: list[int] = []
    ...
    def list_artifact_retention_scheduler_daemon_supervisor_results(
        ...
        *,
        ...
        self.supervisor_collection_statuses.append(response.status_code)
    ...
    def get_artifact_retention_scheduler_daemon_supervisor_detail(
        ...
        *,
        ...
        self.supervisor_detail_statuses.append(response.status_code)
    ...
    def _json_or_error(response: Any) -> dict[str, Any]:
        ...
        if response.status_code >= 400:
            raise AeArtifactOperationsError(
                ...
                detail=payload.get(
                    "detail",
                    "AE artifact retention scheduler daemon supervisor source failed.",
                ),
...
def run_ae_ag_artifact_retention_scheduler_daemon_supervisor_read_model_postgres_smoke(
    ...
    try:
        ...
        execution = _execute_ae_ag_daemon_supervisor_read_model_smoke(
            database_url=database_url,
...
def _execute_ae_ag_daemon_supervisor_read_model_smoke(
    *,
    database_url: str,
    database_env: str,
) -> dict[str, Any]:
    ...
    try:
        ...
        ae_headers = supervisor_pg.artifact_pg._auth_headers(
            request_id=request_id,
            trace_id=trace_id,
        )

        status_response = ae_client.post(
            "/api/v1/artifact-retention/scheduler-daemon-supervisor-controls",
            json=supervisor_pg._supervisor_payload(
                action="status_probe",
                suffix=suffix,
                reason="slice_0569_ag_read_model_status_probe",
            ),
            headers=ae_headers,
        )
        status_payload = supervisor_pg._json_payload(status_response)
        status_record_id = supervisor_pg._record_id(status_payload)
        if status_record_id:
            record_ids.append(status_record_id)

        start_response = ae_client.post(
            "/api/v1/artifact-retention/scheduler-daemon-supervisor-controls",
            json=supervisor_pg._supervisor_payload(
                action="start_daemon",
                suffix=suffix,
                reason="slice_0569_ag_read_model_start_guardrail",
                enabled=True,
        ...
        start_payload = supervisor_pg._json_payload(start_response)
        start_record_id = supervisor_pg._record_id(start_payload)
        if start_record_id:
            record_ids.append(start_record_id)

        scheduler_id = _scheduler_id_from_dispatches(status_payload, start_payload)
        bridge = AeTestClientDaemonSupervisorReadModelClient(
            ae_client,
            headers=ae_headers,
        )
        ...
        ag_detail_response = ag_client.get(
            "/admin/v1/operations/artifact-retention/"
            f"scheduler-daemon-supervisor-results/{start_record_id or 'missing'}",
            headers=ag_headers,
        )
        ag_detail = (
            ag_detail_response.json() if ag_detail_response.status_code == 200 else {}
        )
        db_observations = supervisor_pg._db_observations(
            engine,
            record_ids=record_ids,
            scheduler_id=scheduler_id,
        )
        checks = _ag_supervisor_read_model_checks(
            database_url=database_url,
            database_env=database_env,
        ...
        if failed_checks:
            raise RuntimeError(
                "AE/AG artifact retention scheduler daemon supervisor "
                "read-model PostgreSQL smoke checks failed: "
                f"{', '.join(failed_checks)}"
            )

        cleanup = supervisor_pg._cleanup_supervisor_records(
            supervisor_store,
            record_ids,
        )
        post_cleanup = supervisor_pg._db_observations(
            engine,
            record_ids=record_ids,
            scheduler_id=scheduler_id,
        )
        ...
        return {
            ...
            "trace_id": trace_id,
            "dispatches": {
                "status_probe": supervisor_pg._dispatch_evidence(status_payload),
                "start_daemon": supervisor_pg._dispatch_evidence(start_payload),
            },
            "db_observations": db_observations,
            "routes": {
                "ae_supervisor_collection_statuses": (
                    bridge.supervisor_collection_statuses
                ),
                "ae_supervisor_detail_statuses": bridge.supervisor_detail_statuses,
                "ag_supervisor_collection_status": (
                    ag_collection_response.status_code
                ),
                "ag_supervisor_detail_status": ag_detail_response.status_code,
            },
            "ag_supervisor_collection": {
            ...
            "ag_supervisor_detail": {
                "projection_schema_version": ag_detail.get(
                    "projection_schema_version"
                ),
                "projection_status": ag_detail.get("projection_status"),
                "daemon_supervisor_record_id": ag_detail.get(
                    "daemon_supervisor_record_id"
                ),
                "supervisor_event_count": ag_detail.get("supervisor_event_count"),
        ...
        if record_ids:
            try:
                ...
                supervisor_pg._cleanup_supervisor_records(
                    supervisor_store,
                    record_ids,
                )
...
def _scheduler_id_from_dispatches(
    ...
    start_record = supervisor_pg._mapping_value(start_payload.get("supervisor_record"))
    status_record = supervisor_pg._mapping_value(status_payload.get("supervisor_record"))
    return (
        start_record.get("scheduler_id")
        or status_record.get("scheduler_id")
        or "ae-artifact-retention-scheduler-local-v1"
    )


def _ag_supervisor_read_model_checks(
    *,
    database_url: str,
    database_env: str,
    status_response: int,
    ...
    scheduler_counts = _mapping_value(db_observations.get("scheduler_counts"))
    return {
        "test_database_url": database_env == "NEX_AE_TEST_DATABASE_URL"
        and supervisor_pg._database_name(database_url).endswith("_test"),
        ...
        and start_record.get("process_started") is False,
        "supervisor_rows_persisted": rows
        == {"supervisor_records": 2, "supervisor_events": 2},
        "scheduler_counts_persisted": scheduler_counts
        ...
        and bridge.supervisor_collection_statuses == [200],
        "ag_detail_route_ok": ag_detail_status == 200
        and bridge.supervisor_detail_statuses == [200],
        ...
        and collection_summary.get("supervisor_record_count") == 1
        and collection_summary.get("blocked_count") == 1
        and collection_source.get("source_kind") == "ae_test_client"
        and collection_source.get("supervisor_collection_loaded") is True
        ...
        and ag_detail.get("daemon_supervisor_record_id")
        == start_record.get("daemon_supervisor_record_id")
        and ag_detail.get("supervisor_event_count") == 1
        and detail_summary.get("daemon_supervisor_record_id")
        == start_record.get("daemon_supervisor_record_id")
        and detail_summary.get("result_status") == "BLOCKED"
        and detail_source.get("supervisor_detail_loaded") is True,
        ...
        "metadata_only_evidence": _metadata_only(
            ...
            forbidden_fragments=[
                database_url,
                supervisor_pg._database_url_password(database_url),
                "/data/nex-platform",
                "content_base64",
...
def _safe_detail(detail: str, env: Mapping[str, str]) -> str:
    safe = supervisor_pg._safe_detail(detail, env)
...
def assert_smoke_evidence_redacted(
    ...
    supervisor_pg.assert_smoke_evidence_redacted(serialized_evidence, environ)


def summary_line(evidence: dict[str, Any]) -> str:
    if evidence["status"] == "SKIPPED":
        return (
            "ae_ag_artifact_retention_scheduler_daemon_supervisor_read_model_postgres_smoke="
    ...
    if evidence["status"] == "PASS":
        ...
        return (
            ...
            f"db_records={rows['supervisor_records']} "
            f"events={rows['supervisor_events']} "
            f"ag_collection={evidence['routes']['ag_supervisor_collection_status']} "
            f"ag_detail={evidence['routes']['ag_supervisor_detail_status']} "
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run optional AE/AG artifact retention daemon supervisor read-model "
            "PostgreSQL smoke."
        )
    )
    parser.add_argument(
...
def main(argv: list[str] | None = None) -> int:
    ...
    evidence = (
        run_ae_ag_artifact_retention_scheduler_daemon_supervisor_read_model_postgres_smoke()
    )
