...
SMOKE_ENV = "NEX_DB_SERVICE_LOG_RETENTION_HTTP_SMOKE"
SMOKE_SERVICE_ENV = "NEX_DB_SERVICE_LOG_RETENTION_HTTP_SMOKE_SERVICE"
SMOKE_PROFILE_ENV = "NEX_DB_SERVICE_LOG_RETENTION_HTTP_SMOKE_PROFILE"
DEFAULT_SERVICE_ID = "nex-cx"
DEFAULT_PROFILE = "test"
TRACE_ID = "4bf92f3577b34da6a3ce929d0e0e4736"
REQUEST_ID_PREFIX = "service-log-retention-http-smoke"
JOB_ID = "service-log-retention-http-smoke-job"
LOGGER_NAME = "nex.smoke.service_log_retention_http"
RETENTION_CUTOFF = "2026-07-06T00:00:00Z"
CHECKED_AT = "2026-08-05T00:00:00Z"
RETENTION_DAYS = 30
MAX_DELETE_COUNT = 1
SCHEMA_VERSION = "postgres_service_log_retention_http_smoke.v1"
RETENTION_PATH = "/internal/v1/service-logs/retention/purge"


def run_postgres_service_log_retention_http_smoke(
    ...
    if profile != "test":
        return _failure(
            "profile_not_allowed",
            f"{SMOKE_PROFILE_ENV} must be test for destructive HTTP smoke execution.",
    ...
    try:
        ...
        try:
            ...
            client = _build_service_client(service_id=service_id, store=store)
            unauthorized = _post_json(
                client,
                service_id=service_id,
                request_id=request_id,
                payload={"retention_cutoff": RETENTION_CUTOFF},
                headers={},
            )
            invalid = _post_json(
                client,
            ...
            dry_run = _post_json(
                ...
                payload={
                    ...
                    "idempotency_key": "smoke-retention-http-dry-run",
                },
            )
            blocked = _post_json(
                client,
                ...
                payload={
                    ...
                    "max_delete_count": MAX_DELETE_COUNT,
                    "idempotency_key": "smoke-retention-http-blocked",
                },
            )
            execute = _post_json(
                client,
                ...
                payload={
                    ...
                    "idempotency_key": "smoke-retention-http-execute",
                },
            )
            ...
            if not all(checks.values()):
                return _failure(
                    "checks_failed",
                    "PostgreSQL service log retention HTTP smoke checks failed.",
                    service_id=service_id,
                    profile=profile,
            ...
            evidence = {
                ...
                "path": RETENTION_PATH,
                "http_statuses": {
                    "unauthorized": unauthorized["_http_status"],
                    "invalid": invalid["_http_status"],
            ...
            if not _redaction_safe(evidence):
                return _failure(
                    ...
                    "PostgreSQL service log retention HTTP smoke evidence leaked private data.",
                    service_id=service_id,
                    profile=profile,
                    database_env=database_env,
                )
...
def _build_service_client(
    *,
    ...
    register_service_log_retention_routes(app, service_id=service_id, store=store)
    ...
    *,
    ...
    response = client.post(
        ...
        headers=(
            ...
            else _service_headers(service_id=service_id, request_id=request_id)
        ),
    )
    body = response.json()
...
def _service_headers(*, service_id: str, request_id: str) -> dict[str, str]:
...
def _smoke_log_ids() -> dict[str, str]:
    run_id = uuid4().hex
    return {
        "old_001": f"service-log-retention-http-smoke-old-001-{run_id}",
        "old_002": f"service-log-retention-http-smoke-old-002-{run_id}",
        "fresh": f"service-log-retention-http-smoke-fresh-{run_id}",
    }
    ...
    for key, observed_at in (
        ...
        store.append(
            build_service_log_entry(
                ...
                message="Service log retention HTTP PostgreSQL smoke entry.",
                trace_id=TRACE_ID,
                request_id=request_id,
                job_id=JOB_ID,
                subject_ref={"type": "smoke.retention_http_log", "id": log_id},
...
def _checks(
    *,
    ...
    return {
        "auth_required": (
            unauthorized["_http_status"] == 401
            and unauthorized["error_code"] == "AUTHORIZATION_HEADER_MISSING"
        ),
        "invalid_dry_run_delete_enabled_rejected": (
            invalid["_http_status"] == 422
            and invalid["error_code"]
            == "service_log_retention.delete_enabled_invalid"
        ),
        "dry_run_http_succeeded_without_delete": (
            dry_run["_http_status"] == 200
            and dry_run["retention_execution_schema_version"]
            == SERVICE_LOG_RETENTION_EXECUTION_SCHEMA_VERSION
        ...
        "execute_without_delete_enabled_returns_blocked_execution": (
            blocked["_http_status"] == 200
            and blocked["mode"] == "EXECUTE"
        ...
        "execute_http_deleted_one_candidate": (
            execute["_http_status"] == 200
            and execute["mode"] == "EXECUTE"
            ...
            and execute["request_id"] == dry_run["request_id"]
        ),
        "store_state_guarded": (
...
def summary_line(evidence: dict[str, object]) -> str:
    if evidence["status"] == "SKIPPED":
        return f"postgres_service_log_retention_http_smoke=skipped reason={SMOKE_ENV}"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run optional PostgreSQL service log retention HTTP route smoke."
        )
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    evidence = run_postgres_service_log_retention_http_smoke()
