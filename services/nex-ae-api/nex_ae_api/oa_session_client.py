        ...
        *,
        ...
        try:
            ...
            raise OaUserSessionClientError(
                status_code=504,
                error_code="oa.session_client_timeout",
                detail="OA user-session request timed out.",
                retryable=True,
            ) from exc
            ...
            raise OaUserSessionClientError(
                ...
                detail="OA user-session endpoint is unavailable.",
                retryable=True,
            ) from exc
        ...
        if response.status_code >= 400:
            raise OaUserSessionClientError(
                ...
                error_code=str(body.get("error_code", "oa.session_client_failed")),
...
def _safe_response_json(response: httpx.Response) -> dict[str, Any]:
    try:
        ...
        raise OaUserSessionClientError(
            status_code=response.status_code,
            error_code="oa.session_client_response_invalid",
            detail="OA user-session endpoint did not return valid JSON.",
    ...
    if not isinstance(payload, dict):
        raise OaUserSessionClientError(
            status_code=response.status_code,
            error_code="oa.session_client_response_invalid",
            detail="OA user-session endpoint did not return a JSON object.",
            retryable=response.status_code >= 500,
        )
    return payload
