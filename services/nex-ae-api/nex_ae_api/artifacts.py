...
AE_ARTIFACT_RETENTION_SCHEDULED_JOB_ADMISSION_SCHEMA_VERSION = (
    "ae_artifact_retention_scheduled_job_admission.v1"
)
...
ARTIFACT_RETENTION_SCHEDULER_TICK_STATUSES = ("READY", "NOOP", "SKIPPED")
ARTIFACT_RETENTION_SCHEDULER_TICK_SKIP_REASONS = (
...
def build_default_artifact_retention_scheduler_daemon_supervised_process_store(
    app: Any,
) -> Any | None:
    persistence = getattr(app.state, "nex_persistence", None)
    session_factory = getattr(persistence, "api_session_factory", None)
    ...
    return SqlAlchemyArtifactRetentionSchedulerDaemonSupervisedProcessStore(
        session_factory
    )


def build_default_artifact_retention_scheduler_daemon_operator_control_execution_store(
    app: Any,
) -> Any | None:
    persistence = getattr(app.state, "nex_persistence", None)
    session_factory = getattr(persistence, "api_session_factory", None)
    ...
    return SqlAlchemyArtifactRetentionSchedulerDaemonOperatorControlExecutionStore(
        session_factory
    )
...
def register_artifact_handoff_routes(
    ...
    *,
    ...
    artifact_retention_daemon_supervised_process_store = (
        retention_scheduler_daemon_supervised_process_store
        or build_default_artifact_retention_scheduler_daemon_supervised_process_store(
            app
        )
    )
    ...
    def record_artifact_retention_scheduler_daemon_process_snapshot_route(
        payload: dict[str, Any],
        request: Request,
        authorization: str | None = Header(default=None),
    ):
        ...
        if artifact_retention_daemon_supervised_process_store is None:
            return _artifact_problem_response(
                request,
                ArtifactHandoffError(
                    status_code=503,
                    ...
                    detail=(
                        "AE artifact retention scheduler daemon supervised "
        ...
        try:
            from nex_ae_api.artifact_retention_scheduler_daemon import (
                build_artifact_retention_scheduler_daemon_supervised_process_dispatch,
            ...
            snapshot = (
                build_artifact_retention_scheduler_daemon_supervised_process_snapshot(
                    supervisor_command=command,
                    process_status=optional_text(payload.get("process_status")),
                    process_id=payload.get("process_id"),
                    host_id=optional_text(payload.get("host_id")) or "localhost",
                    observed_at=optional_text(payload.get("observed_at")),
                    started_at=optional_text(payload.get("started_at")),
                    completed_at=optional_text(payload.get("completed_at")),
                    exit_code=payload.get("exit_code"),
                    termination_signal=optional_text(
                        payload.get("termination_signal")
                    ),
                    message=optional_text(payload.get("message")),
                )
            )
            persisted = (
                artifact_retention_daemon_supervised_process_store.record_supervised_process_snapshot(
                    snapshot
                )
            )
            return (
                build_artifact_retention_scheduler_daemon_supervised_process_dispatch(
                    ...
                    supervised_process_record=(
                        persisted["supervised_process_record"]
                    ),
                    supervised_process_event=(
                        persisted["supervised_process_event"]
                    ),
                )
            )
        except ArtifactHandoffError as exc:
    ...
    def list_artifact_retention_scheduler_daemon_process_snapshots(
        ...
        try:
            from nex_ae_api.artifact_retention_scheduler_daemon import (
                build_artifact_retention_scheduler_daemon_supervised_process_collection,
            )

            records = (
                artifact_retention_daemon_supervised_process_store.list_supervised_process_records(
                    scheduler_id=scheduler_id,
                    action=action,
    ...
    def get_artifact_retention_scheduler_daemon_process_snapshot_detail(
        ...
        try:
            from nex_ae_api.artifact_retention_scheduler_daemon import (
                build_artifact_retention_scheduler_daemon_supervised_process_detail,
            )

            supervised_process_record = (
                artifact_retention_daemon_supervised_process_store.get_supervised_process_record(
                    daemon_supervised_process_record_id
                )
            )
            if supervised_process_record is None:
                raise ArtifactHandoffError(
                    status_code=404,
                    error_code=(
                        "ae.artifact_retention_scheduler_daemon_supervised_process_record_not_found"
                    ),
                    detail=(
                        "AE artifact retention scheduler daemon supervised "
                        "process record was not found: "
                        f"{daemon_supervised_process_record_id}"
                    ),
                )
            supervised_process_events = (
                artifact_retention_daemon_supervised_process_store.list_supervised_process_events(
                    supervised_process_record[
                        "daemon_supervised_process_record_id"
                    ]
                )
    ...
    def preview_artifact_retention_scheduler_daemon_operator_control(
        ...
        try:
            return _build_artifact_retention_scheduler_daemon_operator_control_facade_from_route_payload(
                payload,
                artifact_retention_job_queue=artifact_retention_job_queue,
                idempotency_key=idempotency_key,
            )
        except ArtifactHandoffError as exc:
            return _artifact_problem_response(request, exc)
    ...
    def create_artifact_retention_scheduler_daemon_operator_control_execution(
        payload: dict[str, Any],
        request: Request,
        authorization: str | None = Header(default=None),
        idempotency_key: str | None = Header(default=None, alias="Idempotency-Key"),
        ...
        try:
            ...
            persist_execution_state = _boolean_from_payload(
                payload,
            ...
            facade = _build_artifact_retention_scheduler_daemon_operator_control_facade_from_route_payload(
                payload,
                artifact_retention_job_queue=artifact_retention_job_queue,
                idempotency_key=idempotency_key,
            )
            execution_request = (
                build_artifact_retention_scheduler_daemon_operator_control_execution_request(
                    operator_control_facade=facade,
                    execution_mode=(
                        optional_text(payload.get("execution_mode")) or "contract_only"
                    ),
                    requested_at=(
                        optional_text(payload.get("execution_requested_at"))
                        or optional_text(payload.get("requested_at"))
                        or optional_text(payload.get("checked_at"))
                    ),
                )
            )
            existing_execution_state = payload.get("existing_execution_state")
            if persist_execution_state:
                artifact_retention_daemon_operator_control_execution_store.ensure_schema()
            if persist_execution_state and existing_execution_state is None:
                existing_execution_state = (
                    artifact_retention_daemon_operator_control_execution_store.get_execution_state_by_idempotency_key(
            ...
            execution_state = (
                build_artifact_retention_scheduler_daemon_operator_control_execution_state(
                    ...
                    existing_execution_state=existing_execution_state,
                    observed_at=(
                        optional_text(payload.get("observed_at"))
                        or optional_text(payload.get("checked_at"))
                    ),
                )
            )
            if persist_execution_state:
                artifact_retention_daemon_operator_control_execution_store.record_execution_state(
                    execution_state
                )
    ...
    def list_artifact_retention_scheduler_daemon_operator_control_executions(
        ...
        try:
            ...
            records = (
                artifact_retention_daemon_operator_control_execution_store.list_execution_states(
                    scheduler_id=scheduler_id,
                    action=action,
                    execution_status=execution_status,
    ...
    def get_artifact_retention_scheduler_daemon_operator_control_execution_detail(
        ...
        try:
            ...
            transitions = (
                artifact_retention_daemon_operator_control_execution_store.list_execution_state_transitions(
                    operator_control_execution_state_id
                )
            )
            return build_artifact_retention_scheduler_daemon_operator_control_execution_detail(
    ...
    def list_artifact_retention_scheduler_daemon_operator_control_execution_worker_results(
        ...
        if auth_problem is not None:
            return auth_problem
        if artifact_retention_daemon_operator_control_execution_worker_result_store is None:
            return _artifact_problem_response(
                request,
                ArtifactHandoffError(
                    status_code=503,
        ...
        try:
            ...
            records = (
                artifact_retention_daemon_operator_control_execution_worker_result_store.list_worker_results(
                    scheduler_id=scheduler_id,
                    action=action,
            ...
            return build_artifact_retention_scheduler_daemon_operator_control_execution_worker_result_collection(
                records,
                scheduler_id=scheduler_id,
                action=action,
                worker_status=worker_status,
    ...
    def get_artifact_retention_scheduler_daemon_operator_control_execution_worker_result_detail(
        ...
        try:
            ...
            worker_result_record = (
                artifact_retention_daemon_operator_control_execution_worker_result_store.get_worker_result(
                    operator_control_execution_worker_result_id
                )
            )
            if worker_result_record is None:
                raise ArtifactHandoffError(
                    status_code=404,
                    error_code=(
                    ...
                    detail=(
                        "AE artifact retention scheduler daemon operator "
                        "control execution worker result was not found: "
                        f"{operator_control_execution_worker_result_id}"
                    ),
                )
            return build_artifact_retention_scheduler_daemon_operator_control_execution_worker_result_detail(
                worker_result_record
            )
        except ArtifactHandoffError as exc:
            return _artifact_problem_response(request, exc)
    ...
    def transition_artifact_retention_scheduler_daemon_operator_control_execution(
        payload: dict[str, Any],
        request: Request,
        authorization: str | None = Header(default=None),
    ):
        ...
        try:
            ...
            persist_transition = _boolean_from_payload(
                payload,
                "persist_transition",
                default=False,
            )
            ...
            transition = (
                build_artifact_retention_scheduler_daemon_operator_control_execution_state_transition(
                    operator_control_execution_state=payload.get(
                        "operator_control_execution_state"
                    ),
                    target_status=optional_text(payload.get("target_status")) or "",
                    decision_reason=optional_text(payload.get("decision_reason")) or "",
                    transitioned_at=(
                        optional_text(payload.get("transitioned_at"))
                        or optional_text(payload.get("observed_at"))
                    ),
                )
            )
            if persist_transition:
                artifact_retention_daemon_operator_control_execution_store.record_execution_state_transition(
                    transition
                )
            return transition
        except ArtifactHandoffError as exc:
    ...
    *,
    ...
    config = {
        ...
        "scheduler_daemon_process_snapshots": (
            ...
            "scheduler_daemon_operator_control_executions": (
                "/api/v1/artifact-retention/"
                "scheduler-daemon-operator-control-executions"
            ),
            ...
            "scheduler_daemon_operator_control_execution_worker_results": (
                "/api/v1/artifact-retention/"
                "scheduler-daemon-operator-control-execution-worker-results"
            ),
            "scheduler_daemon_operator_control_execution_worker_result_detail": (
                "/api/v1/artifact-retention/"
                "scheduler-daemon-operator-control-execution-worker-results/"
                "{operator_control_execution_worker_result_id}"
            ),
...
def build_artifact_retention_scheduler_tick_plan(
    ...
    *,
    ...
    tick_dt = parse_artifact_retention_timestamp(
        tick_at or _utc_now(),
        field_name="tick_at",
    )
    normalized_tick_at = format_artifact_retention_timestamp(tick_dt)
    in_batch_window = _artifact_retention_scheduler_tick_in_batch_window(
        tick_dt,
        config["runtime"],
    )
    skip_reason = _artifact_retention_scheduler_tick_skip_reason(
        config,
        validated_plan,
        in_batch_window,
    )
    tick_status = _artifact_retention_scheduler_tick_status(skip_reason)
    normalized_idempotency_key = optional_text(
        idempotency_key
    ) or _artifact_retention_scheduler_tick_idempotency_key(
        scheduler_id=config["scheduler_id"],
        plan_id=validated_plan["plan_id"],
        tick_at=normalized_tick_at,
    )
    command_preview = (
        build_artifact_retention_scheduled_execution_command(
            validated_plan,
            trigger_type="scheduler_tick",
            command_created_at=normalized_tick_at,
            requested_by={
                "actor_type": "service",
                "actor_id": "nex-ae-api",
    ...
    tick_plan = {
        ...
        "service_id": "nex-ae-api",
        "scheduler_id": config["scheduler_id"],
        "tick_id": _artifact_retention_scheduler_tick_id(
            scheduler_id=config["scheduler_id"],
            plan_id=validated_plan["plan_id"],
            tick_at=normalized_tick_at,
            idempotency_key=normalized_idempotency_key,
        ...
        "source_plan_summary": summarize_artifact_retention_batch_plan(
            validated_plan
        ),
        "runtime": _artifact_retention_scheduler_tick_runtime_snapshot(
            config,
            in_batch_window=in_batch_window,
        ),
        "admission": {
            "admission_ready": tick_status == "READY",
            "admission_performed": False,
            "trigger_type": "scheduler_tick",
            "requested_at": normalized_tick_at,
            "idempotency_key": normalized_idempotency_key,
            "max_jobs_per_tick": config["runtime"]["scheduler_tick_max_jobs_per_tick"],
        },
    ...
    return validate_artifact_retention_scheduler_tick_plan(tick_plan)


def validate_artifact_retention_scheduler_tick_plan(
    tick_plan: dict[str, Any],
) -> dict[str, Any]:
    if not isinstance(tick_plan, dict):
        raise ArtifactHandoffError(
            status_code=422,
            error_code="ae.artifact_retention_scheduler_tick_plan_invalid",
            detail="Artifact retention scheduler tick plan must be an object.",
        )
    if (
        tick_plan.get("artifact_retention_scheduler_tick_plan_schema_version")
        != AE_ARTIFACT_RETENTION_SCHEDULER_TICK_PLAN_SCHEMA_VERSION
    ):
        raise ArtifactHandoffError(
            status_code=422,
            error_code="ae.artifact_retention_scheduler_tick_plan_schema_invalid",
            detail="Artifact retention scheduler tick plan schema version is invalid.",
        )
    if tick_plan.get("service_id") != "nex-ae-api":
        raise ArtifactHandoffError(
            status_code=422,
            error_code="ae.artifact_retention_scheduler_tick_plan_invalid",
            detail="Artifact retention scheduler tick plan service id is invalid.",
        )
    for section_name in ("source_plan_summary", "runtime", "admission", "metadata"):
        if not isinstance(tick_plan.get(section_name), dict):
            raise ArtifactHandoffError(
                status_code=422,
                error_code="ae.artifact_retention_scheduler_tick_plan_invalid",
                detail=(
                    "Artifact retention scheduler tick plan "
                    f"{section_name} section is required."
                ),
            )
    if optional_text(tick_plan.get("scheduler_id")) is None:
        raise ArtifactHandoffError(
            status_code=422,
            error_code="ae.artifact_retention_scheduler_tick_plan_invalid",
            detail="Artifact retention scheduler tick plan scheduler id is required.",
        )
    if optional_text(tick_plan.get("tick_id")) is None:
        raise ArtifactHandoffError(
            status_code=422,
            error_code="ae.artifact_retention_scheduler_tick_plan_invalid",
            detail="Artifact retention scheduler tick plan tick id is required.",
        )
    if optional_text(tick_plan.get("source_plan_id")) is None:
        raise ArtifactHandoffError(
            status_code=422,
            error_code="ae.artifact_retention_scheduler_tick_plan_invalid",
            detail="Artifact retention scheduler tick plan source plan id is required.",
        )
    parse_artifact_retention_timestamp(tick_plan.get("tick_at"), field_name="tick_at")
    tick_status = tick_plan.get("tick_status")
    skip_reason = tick_plan.get("skip_reason")
    if tick_status not in ARTIFACT_RETENTION_SCHEDULER_TICK_STATUSES:
        raise ArtifactHandoffError(
            status_code=422,
            error_code="ae.artifact_retention_scheduler_tick_plan_invalid",
            detail="Artifact retention scheduler tick plan status is invalid.",
        )
    if not _artifact_retention_scheduler_tick_runtime_snapshot_valid(
        tick_plan["runtime"]
    ):
        raise ArtifactHandoffError(
            status_code=422,
            error_code="ae.artifact_retention_scheduler_tick_plan_invalid",
            detail="Artifact retention scheduler tick plan runtime is invalid.",
        )
    if not _artifact_retention_scheduler_tick_admission_valid(
        tick_plan["admission"],
        ready=tick_status == "READY",
    ):
        raise ArtifactHandoffError(
            status_code=422,
            error_code="ae.artifact_retention_scheduler_tick_plan_invalid",
            detail="Artifact retention scheduler tick plan admission is invalid.",
        )
    if tick_plan["metadata"] != {
        "metadata_only": True,
        "job_enqueued": False,
        "worker_executed": False,
        "history_write_executed": False,
        ...
        raise ArtifactHandoffError(
            status_code=422,
            error_code="ae.artifact_retention_scheduler_tick_plan_invalid",
            detail="Artifact retention scheduler tick plan metadata is invalid.",
        )
    if tick_status == "READY":
        if skip_reason is not None:
            raise ArtifactHandoffError(
                status_code=422,
                error_code="ae.artifact_retention_scheduler_tick_plan_invalid",
                detail="Artifact retention scheduler tick ready plan cannot skip.",
            )
        command = validate_artifact_retention_scheduled_execution_command(
            tick_plan.get("command_preview")
        )
        if (
            command["trigger_type"] != "scheduler_tick"
            or command["source_plan_id"] != tick_plan["source_plan_id"]
            or command["command_created_at"] != tick_plan["tick_at"]
            or command["idempotency_key"] != tick_plan["admission"]["idempotency_key"]
        ):
            raise ArtifactHandoffError(
                status_code=422,
                error_code="ae.artifact_retention_scheduler_tick_plan_invalid",
                detail="Artifact retention scheduler tick command preview is invalid.",
            )
    else:
        if skip_reason not in ARTIFACT_RETENTION_SCHEDULER_TICK_SKIP_REASONS:
            raise ArtifactHandoffError(
                status_code=422,
                error_code="ae.artifact_retention_scheduler_tick_plan_invalid",
                detail="Artifact retention scheduler tick skip reason is invalid.",
            )
        if tick_status == "NOOP" and skip_reason != "no_retention_candidates":
            raise ArtifactHandoffError(
                status_code=422,
                error_code="ae.artifact_retention_scheduler_tick_plan_invalid",
                detail="Artifact retention scheduler tick noop reason is invalid.",
            )
        if tick_status == "SKIPPED" and skip_reason == "no_retention_candidates":
            raise ArtifactHandoffError(
                status_code=422,
                error_code="ae.artifact_retention_scheduler_tick_plan_invalid",
                detail="Artifact retention scheduler tick skipped reason is invalid.",
            )
        if tick_plan.get("command_preview") is not None:
            raise ArtifactHandoffError(
                status_code=422,
                error_code="ae.artifact_retention_scheduler_tick_plan_invalid",
                detail="Artifact retention scheduler tick skipped plan cannot carry command.",
            )
    assert_artifact_retention_payload_safe(tick_plan)
    return tick_plan
...
def _artifact_retention_scheduler_tick_in_batch_window(
    tick_dt: datetime,
    runtime: dict[str, Any],
) -> bool:
    local_dt = tick_dt.astimezone(ZoneInfo(str(runtime["scheduler_tick_timezone"])))
    start = _artifact_retention_scheduler_local_time(
        str(runtime["scheduler_tick_window_start"])
    )
    end = _artifact_retention_scheduler_local_time(
        str(runtime["scheduler_tick_window_end"])
    )
    return start <= local_dt.time() < end


def _artifact_retention_scheduler_local_time(value: str) -> time:
    hour_text, minute_text = value.split(":", 1)
    return time(hour=int(hour_text), minute=int(minute_text))


def _artifact_retention_scheduler_tick_skip_reason(
    scheduler_config: dict[str, Any],
    batch_plan: dict[str, Any],
    in_batch_window: bool,
) -> str | None:
    runtime = scheduler_config["runtime"]
    if runtime["scheduler_tick_admission_enabled"] is not True:
        return "scheduler_tick_admission_disabled"
    if runtime["job_queue_available"] is not True:
        return "job_queue_unavailable"
    if runtime["scheduler_tick_batch_window_enforced"] is True and not in_batch_window:
        return "outside_batch_window"
    if batch_plan["plan_status"] == "NOOP" or batch_plan["selected_count"] == 0:
        return "no_retention_candidates"
    return None


def _artifact_retention_scheduler_tick_status(skip_reason: str | None) -> str:
    if skip_reason is None:
        return "READY"
    if skip_reason == "no_retention_candidates":
        return "NOOP"
    return "SKIPPED"


def _artifact_retention_scheduler_tick_idempotency_key(
    *,
    scheduler_id: str,
    plan_id: str,
    tick_at: str,
) -> str:
    return f"ae-artifact-retention-scheduler-tick:{scheduler_id}:{plan_id}:{tick_at}"


def _artifact_retention_scheduler_tick_id(
    *,
    scheduler_id: str,
    plan_id: str,
    tick_at: str,
    ...
    return str(
        uuid5(
            NAMESPACE_URL,
            f"ae-artifact-retention-scheduler-tick:{sha256_json(basis)}",
        )
    )


def _artifact_retention_scheduler_tick_runtime_snapshot(
    scheduler_config: dict[str, Any],
    *,
    in_batch_window: bool,
) -> dict[str, Any]:
    runtime = scheduler_config["runtime"]
    return {
        "automation_profile": runtime["automation_profile"],
        "scheduler_daemon_enabled": runtime["scheduler_daemon_enabled"],
        "scheduler_daemon_started": False,
        "scheduler_tick_admission_enabled": runtime[
            "scheduler_tick_admission_enabled"
        ],
        "job_queue_available": runtime["job_queue_available"],
        "job_queue_backend": runtime["job_queue_backend"],
        "worker_runner_available": runtime["worker_runner_available"],
        "physical_delete_automation_enabled": runtime[
            "physical_delete_automation_enabled"
        ],
        "default_execution_mode": runtime["default_execution_mode"],
        "tick_interval_seconds": runtime["scheduler_tick_interval_seconds"],
        "tick_jitter_seconds": runtime["scheduler_tick_jitter_seconds"],
        "tick_lock_ttl_seconds": runtime["scheduler_tick_lock_ttl_seconds"],
        "tick_stale_after_seconds": runtime["scheduler_tick_stale_after_seconds"],
        "max_jobs_per_tick": runtime["scheduler_tick_max_jobs_per_tick"],
        "batch_window_enforced": runtime["scheduler_tick_batch_window_enforced"],
        "in_batch_window": in_batch_window,
        "batch_window": {
            "timezone": runtime["scheduler_tick_timezone"],
            "start_local_time": runtime["scheduler_tick_window_start"],
            "end_local_time": runtime["scheduler_tick_window_end"],
        },
    }


def _artifact_retention_scheduler_tick_runtime_snapshot_valid(
    runtime: dict[str, Any],
) -> bool:
    if set(runtime) != {
        "automation_profile",
        ...
        return False
    if runtime.get("automation_profile") != ARTIFACT_RETENTION_SCHEDULER_AUTOMATION_PROFILE:
        return False
    if runtime.get("scheduler_daemon_enabled") is not False:
        return False
    if runtime.get("scheduler_daemon_started") is not False:
        return False
    if runtime.get("scheduler_tick_admission_enabled") is not True:
        return False
    if not isinstance(runtime.get("job_queue_available"), bool):
    ...
    if runtime.get("default_execution_mode") != "DRY_RUN":
        return False
    if runtime.get("tick_interval_seconds") != ARTIFACT_RETENTION_SCHEDULER_TICK_INTERVAL_SECONDS:
        return False
    if runtime.get("tick_jitter_seconds") != ARTIFACT_RETENTION_SCHEDULER_TICK_JITTER_SECONDS:
        return False
    if runtime.get("tick_lock_ttl_seconds") != ARTIFACT_RETENTION_SCHEDULER_TICK_LOCK_TTL_SECONDS:
        return False
    if runtime.get("tick_stale_after_seconds") != ARTIFACT_RETENTION_SCHEDULER_TICK_STALE_AFTER_SECONDS:
        return False
    if runtime.get("max_jobs_per_tick") != ARTIFACT_RETENTION_SCHEDULER_MAX_JOBS_PER_TICK:
        return False
    if runtime.get("batch_window_enforced") is not True:
        return False
    if not isinstance(runtime.get("in_batch_window"), bool):
        return False
    if runtime.get("batch_window") != {
        "timezone": ARTIFACT_RETENTION_BATCH_TIMEZONE,
        "start_local_time": ARTIFACT_RETENTION_BATCH_WINDOW_START,
        "end_local_time": ARTIFACT_RETENTION_BATCH_WINDOW_END,
    }:
...
def _artifact_retention_scheduler_tick_admission_valid(
    ...
    *,
    ...
    return (
        ...
        and admission.get("admission_ready") is ready
        and admission.get("admission_performed") is False
        and admission.get("trigger_type") == "scheduler_tick"
        and optional_text(admission.get("requested_at")) is not None
        and optional_text(admission.get("idempotency_key")) is not None
        and admission.get("max_jobs_per_tick")
        == ARTIFACT_RETENTION_SCHEDULER_MAX_JOBS_PER_TICK
    )
...
def build_artifact_retention_scheduled_job_admission(
    batch_plan: dict[str, Any],
    *,
    trace_id: str,
    request_id: str,
    ...
    validated_plan = validate_artifact_retention_batch_plan(batch_plan)
    normalized_trace_id = _normalize_retention_scheduled_job_trace_id(trace_id)
    ...
    normalized_requested_at = format_artifact_retention_timestamp(
        parse_artifact_retention_timestamp(
            requested_at or command_created_at or validated_plan["checked_at"],
            field_name="requested_at",
        )
    )
    normalized_command_created_at = format_artifact_retention_timestamp(
        parse_artifact_retention_timestamp(
            command_created_at or normalized_requested_at,
            field_name="command_created_at",
        )
    )
    normalized_idempotency_key = (
        optional_text(idempotency_key)
        or artifact_retention_scheduled_job_admission_idempotency_key(
            validated_plan,
            trigger_type=normalized_trigger_type,
        )
    )
    command = build_artifact_retention_scheduled_execution_command(
        validated_plan,
        trigger_type=normalized_trigger_type,
        command_created_at=normalized_command_created_at,
        requested_by=requested_by,
        idempotency_key=normalized_idempotency_key,
    )
    job = (
    ...
    admission = {
        ...
        "admission_status": "READY" if job is not None else "SKIPPED",
        "enqueue_required": job is not None,
        "skip_reason": None if job is not None else "no_retention_candidates",
        "trace_id": normalized_trace_id,
        "request_id": normalized_request_id,
        "requested_at": normalized_requested_at,
        "idempotency_key": normalized_idempotency_key,
        ...
        "job_summary": (
            summarize_artifact_retention_scheduled_job(job)
            if job is not None
            else None
        ),
        "command": command,
...
def validate_artifact_retention_scheduled_job_admission(
    admission: dict[str, Any],
) -> dict[str, Any]:
    if not isinstance(admission, dict):
        raise ArtifactHandoffError(
            ...
            detail="Artifact retention scheduled job admission must be an object.",
        )
    if (
        admission.get(
            "artifact_retention_scheduled_job_admission_schema_version"
        ...
        raise ArtifactHandoffError(
            status_code=422,
            error_code="ae.artifact_retention_scheduled_job_admission_schema_invalid",
            detail="Artifact retention scheduled job admission schema version is invalid.",
        )
    for section_name in ("command_summary", "command", "queue_admission"):
        if not isinstance(admission.get(section_name), dict):
            raise ArtifactHandoffError(
                status_code=422,
                error_code="ae.artifact_retention_scheduled_job_admission_invalid",
                detail=(
                    "Artifact retention scheduled job admission "
                    f"{section_name} is required."
                ),
            )
    command = validate_artifact_retention_scheduled_execution_command(
    ...
    queue_admission = admission["queue_admission"]
    _validate_artifact_retention_scheduled_job_queue_admission(queue_admission)
    for field_name in (
        "source_plan_id",
        "command_id",
        "tenant_id",
        ...
        if admission.get(field_name) != command.get(field_name):
            raise ArtifactHandoffError(
                status_code=422,
                error_code="ae.artifact_retention_scheduled_job_admission_invalid",
                detail=(
                    "Artifact retention scheduled job admission does not "
                    f"match command {field_name}."
                ),
            )
    if admission.get("service_id") != "nex-ae-api":
        raise ArtifactHandoffError(
            status_code=422,
            error_code="ae.artifact_retention_scheduled_job_admission_invalid",
            detail="Artifact retention scheduled job admission service id is invalid.",
        )
    _normalize_retention_scheduled_job_trace_id(admission.get("trace_id"))
    required_string(
        admission,
        "request_id",
        "ae.artifact_retention_scheduled_job_request_id_required",
    ...
    if admission["command_summary"] != (
        summarize_artifact_retention_scheduled_execution_command(command)
    ):
        raise ArtifactHandoffError(
            status_code=422,
            error_code="ae.artifact_retention_scheduled_job_admission_invalid",
            detail="Artifact retention scheduled job admission command summary is invalid.",
        )
    if command["command_status"] == "READY":
        _validate_ready_artifact_retention_scheduled_job_admission(
            admission,
            command,
        )
    else:
        _validate_skipped_artifact_retention_scheduled_job_admission(admission)
    assert_artifact_retention_payload_safe(admission)
    return admission


def enqueue_artifact_retention_scheduled_job(
    queue: JobQueue,
    admission: dict[str, Any],
) -> dict[str, Any]:
    if queue is None or not hasattr(queue, "enqueue"):
        raise ArtifactHandoffError(
            status_code=500,
            error_code="ae.artifact_retention_scheduled_job_queue_invalid",
            detail="Artifact retention scheduled job queue is invalid.",
        )
    validated_admission = validate_artifact_retention_scheduled_job_admission(
        admission
    )
    if not validated_admission["enqueue_required"]:
        return validate_artifact_retention_scheduled_job_enqueue_result(
            _build_artifact_retention_scheduled_job_enqueue_result(
                validated_admission,
                enqueued_job=None,
    ...
    try:
        enqueued_job = queue.enqueue(validated_admission["job"])
    except JobQueueError as exc:
        raise ArtifactHandoffError(
            status_code=exc.status_code,
            error_code="ae.artifact_retention_scheduled_job_admission_failed",
            detail=exc.detail,
...
def validate_artifact_retention_scheduled_job_enqueue_result(
    ...
    if not isinstance(result, dict):
        raise ArtifactHandoffError(
            ...
            detail="Artifact retention scheduled job enqueue result must be an object.",
        )
    if (
        result.get(
        ...
        raise ArtifactHandoffError(
            ...
            detail=(
                "Artifact retention scheduled job enqueue result schema "
                "version is invalid."
            ),
        )
    admission = validate_artifact_retention_scheduled_job_admission(
    ...
    if result.get("service_id") != "nex-ae-api":
        raise ArtifactHandoffError(
            status_code=422,
            error_code="ae.artifact_retention_scheduled_job_enqueue_result_invalid",
            detail="Artifact retention scheduled job enqueue result service id is invalid.",
        )
    for field_name in (
        "source_plan_id",
        "command_id",
        ...
        if result.get(field_name) != admission.get(field_name):
            raise ArtifactHandoffError(
                status_code=422,
                error_code="ae.artifact_retention_scheduled_job_enqueue_result_invalid",
                detail=(
                    "Artifact retention scheduled job enqueue result does "
                    f"not match admission {field_name}."
                ),
            )
    if not isinstance(result.get("queue_admission"), dict):
        raise ArtifactHandoffError(
    ...
    _validate_artifact_retention_scheduled_job_queue_admission(
        result["queue_admission"],
        job_enqueued=result.get("enqueue_status") == "ENQUEUED",
    )
    if admission["enqueue_required"]:
        enqueued_job = validate_artifact_retention_scheduled_job(
            result.get("enqueued_job")
        )
        if (
            result.get("enqueue_status") != "ENQUEUED"
            or result.get("job_enqueued") is not True
            or enqueued_job["idempotency_key"] != admission["idempotency_key"]
        ):
            raise ArtifactHandoffError(
                status_code=422,
                error_code=(
                ...
                detail="Artifact retention scheduled job enqueue result is invalid.",
            )
    elif (
        result.get("enqueue_status") != "SKIPPED"
        or result.get("job_enqueued") is not False
        or result.get("enqueued_job") is not None
    ):
        raise ArtifactHandoffError(
            status_code=422,
            error_code="ae.artifact_retention_scheduled_job_enqueue_result_invalid",
            detail="Artifact retention scheduled job skipped enqueue result is invalid.",
        )
    assert_artifact_retention_payload_safe(result)
    return result


def artifact_retention_scheduled_job_admission_idempotency_key(
    batch_plan: Mapping[str, Any],
    *,
    trigger_type: str = "scheduler_tick",
) -> str:
    validated_plan = validate_artifact_retention_batch_plan(dict(batch_plan))
    normalized_trigger_type = _normalize_artifact_retention_scheduled_trigger(
        trigger_type
    )
    return (
        "ae-artifact-retention-scheduled-job-admission:"
        f"{validated_plan['tenant_id']}:"
        f"{validated_plan['workspace_id']}:"
        f"{validated_plan['owner_user_id']}:"
        f"{validated_plan['plan_id']}:"
        f"{normalized_trigger_type}"
    )


def _build_artifact_retention_scheduled_job_enqueue_result(
    admission: dict[str, Any],
    *,
    enqueued_job: dict[str, Any] | None,
    enqueue_status: str,
) -> dict[str, Any]:
    queue_admission = deepcopy(admission["queue_admission"])
    queue_admission["job_enqueued"] = enqueue_status == "ENQUEUED"
    return {
        "artifact_retention_scheduled_job_enqueue_result_schema_version": (
            AE_ARTIFACT_RETENTION_SCHEDULED_JOB_ENQUEUE_RESULT_SCHEMA_VERSION
        ),
        "service_id": "nex-ae-api",
        "source_plan_id": admission["source_plan_id"],
        "command_id": admission["command_id"],
        "job_id": admission["job_id"],
        "job_type": admission["job_type"],
...
def _validate_ready_artifact_retention_scheduled_job_admission(
    ...
    if not isinstance(admission.get("job"), dict):
        raise ArtifactHandoffError(
            status_code=422,
            error_code="ae.artifact_retention_scheduled_job_admission_invalid",
            detail="Artifact retention scheduled job READY admission is invalid.",
        )
    job = validate_artifact_retention_scheduled_job(admission.get("job"))
    if (
        admission.get("admission_status") != "READY"
        or admission.get("enqueue_required") is not True
        or admission.get("skip_reason") is not None
        or admission.get("job_id") != job["job_id"]
        or admission.get("job_summary")
        != summarize_artifact_retention_scheduled_job(job)
        or job["payload"]["command_id"] != command["command_id"]
        or job["idempotency_key"] != admission["idempotency_key"]
    ):
        raise ArtifactHandoffError(
            status_code=422,
            error_code="ae.artifact_retention_scheduled_job_admission_invalid",
...
def _validate_skipped_artifact_retention_scheduled_job_admission(
    admission: dict[str, Any],
) -> None:
    if (
        admission.get("admission_status") != "SKIPPED"
        or admission.get("enqueue_required") is not False
        or admission.get("skip_reason") != "no_retention_candidates"
        or admission.get("job_id") is not None
        or admission.get("job") is not None
        or admission.get("job_summary") is not None
    ):
        raise ArtifactHandoffError(
            status_code=422,
            error_code="ae.artifact_retention_scheduled_job_admission_invalid",
            detail="Artifact retention scheduled job skipped admission is invalid.",
        )


def _validate_artifact_retention_scheduled_job_queue_admission(
    queue_admission: dict[str, Any],
    *,
...
def _build_artifact_retention_scheduler_daemon_operator_control_facade_from_route_payload(
    payload: Mapping[str, Any],
    *,
    artifact_retention_job_queue: JobQueue | None,
    idempotency_key: str | None,
) -> dict[str, Any]:
    from nex_ae_api.artifact_retention_scheduler_daemon import (
    ...
    operator_request = build_artifact_retention_scheduler_daemon_operator_control_request(
        action=optional_text(payload.get("action")) or "status_probe",
        operator_subject=_operator_control_subject_from_payload(payload),
        idempotency_key=(
        ...
        requested_at=requested_at,
        profile=optional_text(payload.get("profile")) or "test",
        enabled=payload["enabled"] if "enabled" in payload else False,
        explicit_opt_in=(
            payload["explicit_opt_in"] if "explicit_opt_in" in payload else False
        ),
        max_cycles=payload["max_cycles"] if "max_cycles" in payload else 1,
        run_worker=payload["run_worker"] if "run_worker" in payload else False,
