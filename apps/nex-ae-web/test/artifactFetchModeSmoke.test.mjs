...
describe("AE Web artifact fetch-mode smoke script", () => {
  it("emits deterministic same-origin artifact evidence without live network", async () => {
    ...
    assert.equal(evidence.request_observations.fetch_call_count, 5);
    ...
    assert.equal(
      ...
      "ae_web_artifact_fetch_mode_smoke=pass " +
        ...
        "versions=1 fetch_calls=5"
    );
    assert.doesNotMatch(
      serialized,
      /private artifact body|storage_ref|ae:\/\/artifacts|service_token|database_url|provider_url|\/data\/nex-platform/
    );
  });
