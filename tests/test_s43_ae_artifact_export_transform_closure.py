...
import run_s43_ae_artifact_export_transform_closure as closure


ROOT = Path(__file__).resolve().parents[1]


def test_s43_ae_artifact_export_transform_closure_passes_for_repo() -> None:
    evidence = closure.run_s43_ae_artifact_export_transform_closure(ROOT)
...
def test_s43_ae_artifact_export_transform_closure_reports_missing_files(
    tmp_path: Path,
) -> None:
    (tmp_path / "docs" / "slices").mkdir(parents=True)

    evidence = closure.run_s43_ae_artifact_export_transform_closure(tmp_path)
...
def test_s43_ae_artifact_export_transform_closure_reports_token_failures(
    tmp_path: Path,
) -> None:
    ...
    target = tmp_path / "apps" / "nex-ae-web" / "src" / "artifactPreviewPanel.js"
    target.write_text(
        target.read_text(encoding="utf-8").replace(
            "download_payload_kind",
            "downloadPayloadKindDisabled",
        ),
        encoding="utf-8",
    )
    ...
    assert failed == [
        {
            "check_id": "panel_binary_metadata_summary",
            "path": "apps/nex-ae-web/src/artifactPreviewPanel.js",
            "present": False,
        }
    ]


def test_s43_ae_artifact_export_transform_closure_cli_summary_and_json(
    monkeypatch,
    capsys,
) -> None:
    pass_evidence = {
    ...
    monkeypatch.setattr(
        closure,
        "run_s43_ae_artifact_export_transform_closure",
        lambda: pass_evidence,
    )

    assert closure.main(["--summary"]) == 0
    assert "s43_ae_artifact_export_transform_closure=pass" in (
...
def test_s43_ae_artifact_export_transform_closure_read_text_missing(
    tmp_path: Path,
) -> None:
