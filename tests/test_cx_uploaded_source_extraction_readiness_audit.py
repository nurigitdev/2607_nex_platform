...
import run_cx_uploaded_source_extraction_readiness_audit as audit


def protected_env() -> dict[str, str]:
    return {
        "NEX_CX_TEST_DATABASE_URL": (
            "postgresql+psycopg://nex_cx_user:secret-0280@127.0.0.1:5432/nex_cx_test"
        ),
        "NEX_CX_SOURCE_STORAGE_ROOT": "/data/nex-platform/secret-0280/source-files",
...
def test_uploaded_source_extraction_readiness_audit_passes_current_repo() -> None:
    evidence = audit.run_cx_uploaded_source_extraction_readiness_audit({})

    assert evidence["status"] == "PASS"
    assert evidence["audit_schema_version"] == audit.SCHEMA_VERSION
    assert evidence["scope"]["slice"] == "Slice 0280"
    ...
    assert evidence["runtime_probe"]["observations"]["source_bytes_available"] is True
    assert evidence["runtime_probe"]["observations"]["source_checksum_verified"] is True
    assert evidence["runtime_probe"]["observations"]["extraction_artifact_count"] == 1
    assert evidence["checks"]["runtime_probe_passed"] is True
    assert audit.summary_line(evidence).startswith(
        "cx_uploaded_source_extraction_readiness_audit=pass "
    )


def test_uploaded_source_extraction_readiness_audit_does_not_leak_protected_values() -> None:
    env = protected_env()

    evidence = audit.run_cx_uploaded_source_extraction_readiness_audit(env)
    serialized = json.dumps(evidence, ensure_ascii=False)

    assert evidence["status"] == "PASS"
    assert "secret-0280" not in serialized
    assert "browser-secret-0280" not in serialized
    assert "/data/nex-platform" not in serialized
    assert audit.SECRET_SOURCE.decode("utf-8") not in serialized
    assert "nex-cx-uploaded-source-extraction-" not in serialized


def test_uploaded_source_extraction_readiness_audit_reports_missing_boundaries(
    ...
    assert evidence["checks"]["source_bytes_capture_ready"] is False
...
def test_uploaded_source_extraction_readiness_audit_reports_probe_failures() -> None:
    explicit_failure = audit.run_cx_uploaded_source_extraction_readiness_audit(
        {},
        runtime_probe_runner=lambda: {"status": "FAIL", "failure_code": "probe_bad"},
    )
    exception_failure = audit.run_cx_uploaded_source_extraction_readiness_audit(
        {},
        runtime_probe_runner=lambda: (_ for _ in ()).throw(RuntimeError("boom")),
    )
    invalid_failure = audit.run_cx_uploaded_source_extraction_readiness_audit(
        {},
...
def test_uploaded_source_extraction_readiness_audit_helpers_and_cli(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    ...
    with pytest.raises(ValueError, match="temp path"):
        audit.assert_evidence_redacted("nex-cx-uploaded-source-extraction-abcd", {})
    ...
    assert "cx_uploaded_source_extraction_readiness_audit=pass" in capsys.readouterr().out
...
def test_uploaded_source_extraction_readiness_audit_quality_gate_docs_wired() -> None:
    ...
    assert "run_cx_uploaded_source_extraction_readiness_audit.py --summary" in quality_gate
    assert "0280_cx_uploaded_source_extraction_readiness_audit.md" in docs_index
