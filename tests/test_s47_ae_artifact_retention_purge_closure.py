...
import run_s47_ae_artifact_retention_purge_closure as closure


ROOT = Path(__file__).resolve().parents[1]


def test_s47_ae_artifact_retention_purge_closure_passes_for_repo() -> None:
    evidence = closure.run_s47_ae_artifact_retention_purge_closure(ROOT)
...
def test_s47_ae_artifact_retention_purge_closure_reports_missing_files(
    tmp_path: Path,
) -> None:
    (tmp_path / "docs" / "slices").mkdir(parents=True)

    evidence = closure.run_s47_ae_artifact_retention_purge_closure(tmp_path)
...
def test_s47_ae_artifact_retention_purge_closure_reports_token_failures(
    tmp_path: Path,
) -> None:
    ...
    assert failed == [
        {
            "check_id": "retention_purge_db_after_execute",
            "path": "scripts/smoke/run_ae_artifact_retention_purge_postgres_smoke.py",
            "present": False,
        }
    ]


def test_s47_ae_artifact_retention_purge_closure_cli_summary_and_json(
    monkeypatch,
    capsys,
) -> None:
    pass_evidence = {
    ...
    monkeypatch.setattr(
        closure,
        "run_s47_ae_artifact_retention_purge_closure",
        lambda: pass_evidence,
    )

    assert closure.main(["--summary"]) == 0
    assert "s47_ae_artifact_retention_purge_closure=pass" in capsys.readouterr().out
...
def test_s47_ae_artifact_retention_purge_closure_read_text_missing(
    tmp_path: Path,
) -> None:
