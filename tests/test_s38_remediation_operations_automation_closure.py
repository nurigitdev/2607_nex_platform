...
import run_s38_remediation_operations_automation_closure as closure


ROOT = Path(__file__).resolve().parents[1]


def test_s38_remediation_operations_automation_closure_passes_for_repo() -> None:
    evidence = closure.run_s38_remediation_operations_automation_closure(ROOT)
...
def test_s38_remediation_operations_automation_closure_reports_missing_files(
    tmp_path: Path,
) -> None:
    (tmp_path / "docs" / "slices").mkdir(parents=True)

    evidence = closure.run_s38_remediation_operations_automation_closure(tmp_path)
...
def test_s38_remediation_operations_automation_closure_reports_token_failures(
    tmp_path: Path,
) -> None:
    ...
    assert failed == [
        {
            "check_id": "ae_repaired_response_handoff_openapi",
...
def test_s38_remediation_operations_automation_closure_cli_summary_and_json(
    monkeypatch,
    capsys,
) -> None:
    pass_evidence = {
        ...
        "required_file_count": 41,
        "checks": {},
    }
    monkeypatch.setattr(
        closure,
        "run_s38_remediation_operations_automation_closure",
        lambda: pass_evidence,
    )

    assert closure.main(["--summary"]) == 0
    assert "s38_remediation_operations_automation_closure=pass" in (
