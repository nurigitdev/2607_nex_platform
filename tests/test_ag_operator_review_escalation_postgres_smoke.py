...
import run_ag_operator_review_escalation_postgres_smoke as smoke
...
def test_ag_operator_review_escalation_postgres_smoke_skips_when_disabled() -> None:
    evidence = smoke.run_ag_operator_review_escalation_postgres_smoke({})
...
def test_ag_operator_review_escalation_postgres_smoke_requires_database_url() -> None:
    evidence = smoke.run_ag_operator_review_escalation_postgres_smoke(
        {smoke.SMOKE_ENV: "1"}
    )
...
def test_ag_operator_review_escalation_postgres_smoke_reports_migration_failure(
    ...
    evidence = smoke.run_ag_operator_review_escalation_postgres_smoke(smoke_env())
...
def test_ag_operator_review_escalation_postgres_smoke_success_is_redacted(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    monkeypatch.setattr(
        smoke,
        "_cleanup_smoke_rows",
        lambda *args, **kwargs: {"cases": 1, "escalations": 1, "events": 1},
    )
    env = smoke_env()

    evidence = smoke.run_ag_operator_review_escalation_postgres_smoke(env)
    serialized = str(evidence)

    assert evidence["status"] == "PASS"
    assert evidence["checks"]["list_count"] is True
    assert evidence["checks"]["dashboard_action_required"] is True
    assert evidence["checks"]["issue_candidate_count"] is True
    assert evidence["checks"]["action_acknowledged"] is True
    assert evidence["checks"]["replay_idempotent"] is True
    assert evidence["cleanup"] == {"cases": 1, "escalations": 1, "events": 1}
    ...
    assert "ag-op-escalation-action-idem" not in serialized
    assert smoke.summary_line(evidence).startswith(
        "ag_operator_review_escalation_postgres_smoke=pass"
    )


def test_ag_operator_review_escalation_postgres_smoke_reports_failed_checks(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    fake_engine = FakeEngine()
    monkeypatch.setattr(smoke, "run_service_migrations", _migration_result)
    ...
    monkeypatch.setattr(
        smoke,
        "_cleanup_smoke_rows",
        lambda *args, **kwargs: {"cases": 1, "escalations": 1, "events": 0},
    )
    ...
    assert smoke.summary_line(evidence) == (
        "ag_operator_review_escalation_postgres_smoke=fail reason=checks_failed"
    )


def test_ag_operator_review_escalation_postgres_smoke_reports_execution_error(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    fake_engine = FakeEngine()
    monkeypatch.setattr(smoke, "run_service_migrations", _migration_result)
...
def test_ag_operator_review_escalation_postgres_smoke_handles_engine_error(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(smoke, "run_service_migrations", _migration_result)
    monkeypatch.setattr(
...
def test_ag_operator_review_escalation_postgres_smoke_helpers() -> None:
    assert smoke._regclass_matches("public.ag_op_escalations", "ag_op_escalations")
    assert not smoke._regclass_matches(None, "ag_op_escalations")
    assert smoke._db_observations(
        ...
        action_id="action",
        record_idempotency_key="record-idem",
        action_idempotency_key="action-idem",
    ...
    assert smoke._cleanup_smoke_rows(
        ...
        "target",
        "trace",
    ) == {"cases": 1, "escalations": 1, "events": 2}
    assert cleanup_engine.connection.calls == 3
    assert smoke._cleanup_smoke_rows(object(), None, None, None, None) == {
        "cases": 0,
        "escalations": 0,
        "events": 0,
    }
    assert smoke._cleanup_smoke_rows(object(), "case", "escalation", "target", "trace") == {
        "cases": 0,
        "escalations": 0,
        "events": 0,
    }
    ...
    with pytest.raises(ValueError):
        smoke.assert_smoke_evidence_redacted(
            smoke.RAW_ACTION_COMMENT,
            {},
            forbidden_values=(smoke.RAW_ACTION_COMMENT,),
        )


def test_ag_operator_review_escalation_postgres_smoke_main(
    monkeypatch: pytest.MonkeyPatch,
    ...
    monkeypatch.setattr(
        smoke,
        "run_ag_operator_review_escalation_postgres_smoke",
        lambda: skipped,
    )

    assert smoke.main(["--summary"]) == 0
    assert "ag_operator_review_escalation_postgres_smoke=skipped" in (
...
def _migration_result(*args: object, **kwargs: object) -> SimpleNamespace:
    return SimpleNamespace(
        ...
        planned=("0692_ag_operator_review_escalation_persistence",),
        applied=("0699_ag_operator_review_escalation_postgres_smoke",),
        skipped=(),
    )
...
def _observations_without_tables() -> dict[str, object]:
    return {
        ...
        "event_count": 1,
        "event_details_escalation_count": 1,
