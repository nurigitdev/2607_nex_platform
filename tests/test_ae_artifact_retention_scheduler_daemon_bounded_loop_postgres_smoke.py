...
import run_ae_artifact_retention_scheduler_daemon_bounded_loop_postgres_smoke as smoke
...
def smoke_env() -> dict[str, str]:
    return {
        ...
        "NEX_AE_TEST_DATABASE_URL": (
            "postgresql+psycopg://nex_ae_user:secret-0545@127.0.0.1:5432/"
            "nex_ae_test"
        ),
    }


def test_ae_artifact_retention_scheduler_daemon_bounded_loop_postgres_smoke_skips() -> (
    None
):
    evidence = (
        smoke.run_ae_artifact_retention_scheduler_daemon_bounded_loop_postgres_smoke(
            {}
        )
    )
...
def test_ae_artifact_retention_scheduler_daemon_bounded_loop_rejects_dev_profile() -> (
    None
):
    evidence = (
        smoke.run_ae_artifact_retention_scheduler_daemon_bounded_loop_postgres_smoke(
...
def test_ae_artifact_retention_scheduler_daemon_bounded_loop_missing_db_url() -> None:
    evidence = (
        smoke.run_ae_artifact_retention_scheduler_daemon_bounded_loop_postgres_smoke(
            {smoke.SMOKE_ENV: "1"}
        )
...
def test_ae_artifact_retention_scheduler_daemon_bounded_loop_migration_failure(
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: (_ for _ in ()).throw(
            MigrationError("bad secret-0545")
        ),
    )
    ...
    assert evidence["status"] == "FAIL"
    assert evidence["failure_code"] == "configuration_invalid"
    assert "secret-0545" not in evidence["detail"]


def test_ae_artifact_retention_scheduler_daemon_bounded_loop_passes_sqlite_harness(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    assert evidence["bounded_loop"] == {
        "schema_version": (
            smoke.AE_ARTIFACT_RETENTION_SCHEDULER_DAEMON_BOUNDED_LOOP_RESULT_SCHEMA_VERSION
        ),
    ...
    assert [key.endswith(f":cycle:{index}") for index, key in enumerate(
        evidence["jobs"]["idempotency_keys"],
        start=1,
    )] == [True, True]
    ...
    assert "secret-0545" not in serialized
    assert "/data/nex-platform" not in serialized
    assert "storage_ref" not in serialized


def test_ae_artifact_retention_scheduler_daemon_bounded_loop_failed_checks(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    engine = sqlite_artifact_session_factory().kw["bind"]
    monkeypatch.setattr(smoke, "build_engine", lambda _database_url: engine)
    ...
    with pytest.raises(RuntimeError, match="storage_files_retained"):
        smoke._execute_ae_artifact_retention_scheduler_daemon_bounded_loop_smoke(
            database_url=smoke_env()["NEX_AE_TEST_DATABASE_URL"],
            database_env="NEX_AE_TEST_DATABASE_URL",
        )


def test_ae_artifact_retention_scheduler_daemon_bounded_loop_execution_failure(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        smoke,
    ...
    monkeypatch.setattr(
        smoke,
        "_execute_ae_artifact_retention_scheduler_daemon_bounded_loop_smoke",
...
def test_ae_artifact_retention_scheduler_daemon_bounded_loop_helpers() -> None:
    env = smoke_env()
    ...
    with pytest.raises(ValueError, match="database password"):
        smoke.assert_smoke_evidence_redacted("secret-0545", env)
    with pytest.raises(ValueError, match="local data path"):
        smoke.assert_smoke_evidence_redacted("/data/nex-platform/private", {})


def test_ae_artifact_retention_scheduler_daemon_bounded_loop_main_outputs(
    monkeypatch: pytest.MonkeyPatch,
    ...
    monkeypatch.setattr(
        smoke,
        "run_ae_artifact_retention_scheduler_daemon_bounded_loop_postgres_smoke",
        lambda: skipped,
    )

    assert smoke.main(["--summary"]) == 0
    assert (
        "ae_artifact_retention_scheduler_daemon_bounded_loop_postgres_smoke=skipped"
