...
import run_s72_operator_review_escalation_dispatch_execution_closure as closure


def test_s72_operator_review_escalation_dispatch_execution_closure_passes_repo() -> None:
    evidence = closure.run_s72_operator_review_escalation_dispatch_execution_closure()

    assert evidence["status"] == "PASS"
    assert evidence["slice_range"] == "0712-0720"
    assert (
        evidence["boundary"]
        == "ag_owned_operator_review_escalation_dispatch_execution_worker"
    )
    assert evidence["persisted_table"] == "ag_op_esc_dispatches"
    assert evidence["worker_mode"] == "bounded_run_once"
    ...
    assert evidence["result_storage"] == "safe_hashes_statuses_counters_only"
    assert evidence["dashboard_visibility"] == (
        "execution_summary_and_item_result_projection"
    )
...
def test_s72_operator_review_escalation_dispatch_execution_closure_summary() -> None:
    evidence = closure.run_s72_operator_review_escalation_dispatch_execution_closure()
    ...
    assert "slice_range=0712-0720" in summary
    assert "table=ag_op_esc_dispatches" in summary
    assert "worker=bounded_run_once" in summary
    assert "provider=mock_first_only" in summary
    assert "smoke=test_db_protected" in summary


def test_s72_operator_review_escalation_dispatch_execution_closure_reports_missing_file(
    tmp_path: Path,
) -> None:
    ...
    assert evidence["failure_code"] == "s72_closure_failed"
...
def test_s72_operator_review_escalation_dispatch_execution_closure_reports_token_failure(
    tmp_path: Path,
) -> None:
    root = _minimal_root(tmp_path)
...
def test_s72_operator_review_escalation_dispatch_execution_closure_main_summary(
...
def test_s72_operator_review_escalation_dispatch_execution_closure_main_json(
    capsys: pytest.CaptureFixture[str],
) -> None:
