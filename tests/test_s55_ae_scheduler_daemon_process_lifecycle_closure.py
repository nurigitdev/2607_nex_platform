...
import run_s55_ae_scheduler_daemon_process_lifecycle_closure as closure


ROOT = Path(__file__).resolve().parents[1]


def test_s55_ae_scheduler_daemon_process_lifecycle_closure_passes_for_repo() -> None:
    evidence = closure.run_s55_ae_scheduler_daemon_process_lifecycle_closure(ROOT)
    ...
    assert evidence["experience_matrix"] == {
        "process_boundary_audit": True,
        "runtime_state_contract_schema": True,
        "cli_entrypoint_foundation": True,
        "bounded_loop_adapter": True,
        "bounded_loop_postgresql_smoke": True,
        "graceful_shutdown_state_transition": True,
        "retry_backoff_circuit_guard": True,
        "ag_lifecycle_projection": True,
        "ag_lifecycle_postgresql_smoke": True,
    ...
    assert closure.summary_line(evidence).startswith(
        "s55_ae_scheduler_daemon_process_lifecycle_closure=pass"
    )
    assert "process_boundary=ae_owned" in closure.summary_line(evidence)
    assert "lifecycle=RUNNING" in closure.summary_line(evidence)


def test_s55_ae_scheduler_daemon_process_lifecycle_closure_reports_missing_files(
    tmp_path: Path,
) -> None:
    (tmp_path / "docs" / "slices").mkdir(parents=True)

    evidence = closure.run_s55_ae_scheduler_daemon_process_lifecycle_closure(tmp_path)
...
def test_s55_ae_scheduler_daemon_process_lifecycle_closure_reports_token_failures(
    tmp_path: Path,
) -> None:
    ...
    target.write_text(
        target.read_text(encoding="utf-8").replace(
            ...
            "AG_ARTIFACT_OPERATION_RETENTION_DAEMON_LIFECYCLE_SCHEMA_VERSION",
        ),
        encoding="utf-8",
    )
    ...
    assert failed == [
        {
            "check_id": "ag_lifecycle_projection_schema",
...
def test_s55_ae_scheduler_daemon_process_lifecycle_closure_reports_redaction_failure(
    tmp_path: Path,
) -> None:
    for relative_path in closure.REQUIRED_FILES:
        source = ROOT / relative_path
...
def test_s55_ae_scheduler_daemon_process_lifecycle_closure_cli_summary_and_json(
    monkeypatch,
    capsys,
) -> None:
    pass_evidence = {
    ...
    monkeypatch.setattr(
        closure,
        "run_s55_ae_scheduler_daemon_process_lifecycle_closure",
        lambda: pass_evidence,
    )

    assert closure.main(["--summary"]) == 0
    assert "s55_ae_scheduler_daemon_process_lifecycle_closure=pass" in (
...
def test_s55_ae_scheduler_daemon_process_lifecycle_closure_read_text_missing(
    tmp_path: Path,
) -> None:
