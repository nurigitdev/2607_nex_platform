...
def test_ag_artifact_retention_automation_operations_smoke_passes() -> None:
    ...
    assert evidence["worker_result_collection"]["summary"][
        "operator_control_execution_worker_result_count"
    ] == 2
    assert evidence["worker_result_collection"]["summary"]["failed_count"] == 1
    assert evidence["worker_result_detail"]["summary"]["worker_status"] == "SUCCEEDED"
    ...
    assert evidence["checks"]["worker_result_collection_visible"] is True
    assert evidence["checks"]["worker_result_detail_visible"] is True
...
def test_ag_artifact_retention_automation_operations_smoke_redaction_guard() -> None:
    ...
    assert malformed["worker_result_collection_visible"] is False
    assert malformed["worker_result_detail_visible"] is False
...
def test_ag_artifact_retention_automation_operations_smoke_cli_and_output(
    ...
    assert "worker_results=2" in summary_output
    assert "worker_result_detail=SUCCEEDED" in summary_output
