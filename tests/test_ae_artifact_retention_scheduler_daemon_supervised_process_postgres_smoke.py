...
import run_ae_artifact_retention_scheduler_daemon_supervised_process_postgres_smoke as smoke
...
def smoke_env() -> dict[str, str]:
    return {
        ...
        "NEX_AE_TEST_DATABASE_URL": (
            "postgresql+psycopg://nex_ae_user:secret-0575@127.0.0.1:5432/"
            "nex_ae_test"
        ),
    }
...
def test_ae_artifact_retention_scheduler_daemon_supervised_process_smoke_skips() -> (
    None
):
    evidence = (
        smoke.run_ae_artifact_retention_scheduler_daemon_supervised_process_postgres_smoke(
            {}
        )
    )
...
def test_ae_artifact_retention_scheduler_daemon_supervised_process_rejects_dev_profile() -> (
    None
):
    evidence = (
        smoke.run_ae_artifact_retention_scheduler_daemon_supervised_process_postgres_smoke(
...
def test_ae_artifact_retention_scheduler_daemon_supervised_process_missing_db_url() -> (
    None
):
    evidence = (
        smoke.run_ae_artifact_retention_scheduler_daemon_supervised_process_postgres_smoke(
...
def test_ae_artifact_retention_scheduler_daemon_supervised_process_migration_failure(
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: (_ for _ in ()).throw(
            MigrationError("bad secret-0575")
        ),
    )
    ...
    assert evidence["status"] == "FAIL"
    assert evidence["failure_code"] == "configuration_invalid"
    assert "secret-0575" not in evidence["detail"]


def test_ae_artifact_retention_scheduler_daemon_supervised_process_passes_sqlite_harness(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    assert evidence["routes"] == {
        "missing_dispatch_status": 200,
        "running_dispatch_status": 200,
        "collection_status": 200,
        "detail_status": 200,
    }
    assert evidence["dispatches"]["status_probe_missing"]["schema_version"] == (
        smoke.AE_ARTIFACT_RETENTION_SCHEDULER_DAEMON_SUPERVISED_PROCESS_DISPATCH_SCHEMA_VERSION
    )
    assert evidence["dispatches"]["status_probe_missing"]["process_status"] == (
        "MISSING"
    )
    assert evidence["dispatches"]["status_probe_missing"]["process_running"] is False
    assert evidence["dispatches"]["start_daemon_running"]["process_status"] == (
        "RUNNING"
    )
    assert evidence["dispatches"]["start_daemon_running"]["process_id"] == 57575
    assert evidence["dispatches"]["start_daemon_running"]["process_running"] is True
    assert evidence["dispatches"]["start_daemon_running"]["process_control_allowed"] is (
        False
    )
    assert evidence["dispatches"]["start_daemon_running"]["event_type"] == (
        "SUPERVISED_PROCESS_SNAPSHOT_RECORDED"
    )
    assert evidence["collection"]["schema_version"] == (
        smoke.AE_ARTIFACT_RETENTION_SCHEDULER_DAEMON_SUPERVISED_PROCESS_COLLECTION_SCHEMA_VERSION
    )
    assert evidence["collection"]["count"] == 1
    assert evidence["collection"]["actions"] == ["start_daemon"]
    assert evidence["collection"]["process_statuses"] == ["RUNNING"]
    assert evidence["detail"]["schema_version"] == (
        smoke.AE_ARTIFACT_RETENTION_SCHEDULER_DAEMON_SUPERVISED_PROCESS_DETAIL_SCHEMA_VERSION
    )
    assert evidence["detail"]["supervised_process_event_count"] == 1
    assert evidence["detail"]["event_types"] == [
        "SUPERVISED_PROCESS_SNAPSHOT_RECORDED"
    ]
    ...
    assert evidence["db_observations"]["record_counts"] == {
        "status_probe_missing": 1,
        "start_daemon_running": 1,
        "running_pid": 1,
    }
    ...
    assert "secret-0575" not in serialized
    assert "/data/nex-platform" not in serialized
    assert "postgresql+psycopg://nex_ae_user:secret-0575" not in serialized


def test_ae_artifact_retention_scheduler_daemon_supervised_process_failed_checks(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    engine = sqlite_artifact_session_factory().kw["bind"]
    monkeypatch.setattr(smoke, "build_engine", lambda _database_url: engine)
    ...
    with pytest.raises(RuntimeError, match="process_rows_persisted"):
        smoke._execute_ae_artifact_retention_scheduler_daemon_supervised_process_smoke(
            database_url=smoke_env()["NEX_AE_TEST_DATABASE_URL"],
            database_env="NEX_AE_TEST_DATABASE_URL",
        )


def test_ae_artifact_retention_scheduler_daemon_supervised_process_cleanup_failure(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    engine = sqlite_artifact_session_factory().kw["bind"]
    observations = iter(
...
def test_ae_artifact_retention_scheduler_daemon_supervised_process_wraps_value_error(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    engine = sqlite_artifact_session_factory().kw["bind"]
    monkeypatch.setattr(smoke, "build_engine", lambda _database_url: engine)
...
def test_ae_artifact_retention_scheduler_daemon_supervised_process_finally_swallows_cleanup(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    engine = sqlite_artifact_session_factory().kw["bind"]
    monkeypatch.setattr(smoke, "build_engine", lambda _database_url: engine)
...
def test_ae_artifact_retention_scheduler_daemon_supervised_process_execution_failure(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        smoke,
    ...
    monkeypatch.setattr(
        smoke,
        "_execute_ae_artifact_retention_scheduler_daemon_supervised_process_smoke",
...
def test_ae_artifact_retention_scheduler_daemon_supervised_process_helpers(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    env = smoke_env()
    ...
    assert "secret-0575" not in smoke._safe_detail(
        env["NEX_AE_TEST_DATABASE_URL"],
        env,
    )
    with pytest.raises(ValueError, match="database password"):
        smoke.assert_smoke_evidence_redacted("secret-0575", env)
...
def test_ae_artifact_retention_scheduler_daemon_supervised_process_helper_edges() -> (
    None
):
    class RaisesJson:
    ...
    assert smoke._record_id(
        {"supervised_process_record": {"daemon_supervised_process_record_id": ""}}
    ) is None
    ...
    assert smoke._detail_evidence({"supervised_process_events": "wrong"}) == {
        "schema_version": None,
        "record_id": None,
        "supervised_process_event_count": None,
        "event_types": [],
    }


def test_ae_artifact_retention_scheduler_daemon_supervised_process_postgres_helpers() -> (
    None
):
    class FakeResult:
        def __init__(
            self,
            *,
            scalar_value: str | int | None = None,
    ...
    assert smoke._scalar_count(connection, "SELECT count(*)", {}) == 0


def test_ae_artifact_retention_scheduler_daemon_supervised_process_main_outputs(
    monkeypatch: pytest.MonkeyPatch,
