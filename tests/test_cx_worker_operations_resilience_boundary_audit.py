...
def test_repository_worker_operations_resilience_boundary_passes() -> None:
    ...
    assert result["gap_states"]["bounded_cancellation_runtime_missing"] == (
        "RESOLVED"
    )
    assert result["gap_states"]["retry_poison_operations_missing"] == (
        "RESOLVED"
    )
...
def test_worker_operations_boundary_helpers_and_main(monkeypatch, capsys) -> None:
    ...
    assert audit.summary_line(passing) == (
        ...
        "open=0 scope=cx_worker_operations_and_resilience "
