...
import run_s37_remediation_runtime_integration_closure as closure


ROOT = Path(__file__).resolve().parents[1]


def test_s37_remediation_runtime_integration_closure_passes_for_repo() -> None:
    evidence = closure.run_s37_remediation_runtime_integration_closure(ROOT)
...
def test_s37_remediation_runtime_integration_closure_reports_missing_files(
    tmp_path: Path,
) -> None:
    (tmp_path / "docs" / "slices").mkdir(parents=True)

    evidence = closure.run_s37_remediation_runtime_integration_closure(tmp_path)
...
def test_s37_remediation_runtime_integration_closure_reports_token_failures(
    tmp_path: Path,
) -> None:
    ...
    target = tmp_path / "contracts/openapi/nex-ag.openapi.yaml"
    target.write_text(
        target.read_text(encoding="utf-8").replace(
            "syncAgGenerationRemediationExecutionStatus",
            "syncRemediationExecutionStatusDisabled",
        ),
        encoding="utf-8",
    )
    ...
    assert failed == [
        {
            "check_id": "ag_status_sync_route_contract",
            "path": "contracts/openapi/nex-ag.openapi.yaml",
            "present": False,
        }
    ]


def test_s37_remediation_runtime_integration_closure_cli_summary_and_json(
    monkeypatch,
    capsys,
) -> None:
    pass_evidence = {
    ...
    assert "s37_remediation_runtime_integration_closure=pass" in (
