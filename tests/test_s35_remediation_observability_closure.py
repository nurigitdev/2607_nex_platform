...
import run_s35_remediation_observability_closure as closure


ROOT = Path(__file__).resolve().parents[1]


def test_s35_remediation_observability_closure_passes_for_repo() -> None:
    evidence = closure.run_s35_remediation_observability_closure(ROOT)
...
def test_s35_remediation_observability_closure_reports_missing_files(
    tmp_path: Path,
) -> None:
    (tmp_path / "docs" / "slices").mkdir(parents=True)

    evidence = closure.run_s35_remediation_observability_closure(tmp_path)
...
def test_s35_remediation_observability_closure_reports_token_failures(
    tmp_path: Path,
) -> None:
    ...
    target.write_text(
        target.read_text(encoding="utf-8").replace(
            ...
            "generation_remediation_attention_missing.v1",
        ),
        encoding="utf-8",
    )
    ...
    assert failed == [
        {
            "check_id": "operations_issue_candidate_rule",
...
def test_s35_remediation_observability_closure_cli_summary_and_json(
    monkeypatch,
    capsys,
) -> None:
    pass_evidence = {
        ...
        "required_file_count": 26,
        "checks": {},
    }
    monkeypatch.setattr(
        closure,
        "run_s35_remediation_observability_closure",
        lambda: pass_evidence,
    )

    assert closure.main(["--summary"]) == 0
    assert "s35_remediation_observability_closure=pass" in capsys.readouterr().out
