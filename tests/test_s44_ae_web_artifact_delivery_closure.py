...
import run_s44_ae_web_artifact_delivery_closure as closure


ROOT = Path(__file__).resolve().parents[1]


def test_s44_ae_web_artifact_delivery_closure_passes_for_repo() -> None:
    evidence = closure.run_s44_ae_web_artifact_delivery_closure(ROOT)
...
def test_s44_ae_web_artifact_delivery_closure_reports_missing_files(
    tmp_path: Path,
) -> None:
    (tmp_path / "docs" / "slices").mkdir(parents=True)

    evidence = closure.run_s44_ae_web_artifact_delivery_closure(tmp_path)
...
def test_s44_ae_web_artifact_delivery_closure_reports_token_failures(
    tmp_path: Path,
) -> None:
    ...
    target = (
        ...
        / "nex-ae-web"
        / "src"
        / "artifactDownloadFormatSelector.js"
    )
    target.write_text(
        target.read_text(encoding="utf-8").replace(
            "renderArtifactDownloadFormatSelector",
            "renderDisabledArtifactDownloadFormatSelector",
        ),
        encoding="utf-8",
    )
    ...
    assert failed == [
        {
            "check_id": "download_format_selector_renderer",
            "path": "apps/nex-ae-web/src/artifactDownloadFormatSelector.js",
            "present": False,
        }
    ]


def test_s44_ae_web_artifact_delivery_closure_cli_summary_and_json(
    monkeypatch,
    capsys,
) -> None:
    pass_evidence = {
    ...
    monkeypatch.setattr(
        closure,
        "run_s44_ae_web_artifact_delivery_closure",
        lambda: pass_evidence,
    )

    assert closure.main(["--summary"]) == 0
    assert "s44_ae_web_artifact_delivery_closure=pass" in capsys.readouterr().out
...
def test_s44_ae_web_artifact_delivery_closure_read_text_missing(
    tmp_path: Path,
) -> None:
