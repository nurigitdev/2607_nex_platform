...
import run_ae_artifact_retention_scheduled_worker_postgres_smoke as smoke
...
def smoke_env() -> dict[str, str]:
    return {
        ...
        "NEX_AE_TEST_DATABASE_URL": (
            "postgresql+psycopg://nex_ae_user:secret-0495@127.0.0.1:5432/"
            "nex_ae_test"
        ),
    }
...
def test_ae_artifact_retention_scheduled_worker_postgres_smoke_skips_when_disabled() -> None:
    evidence = smoke.run_ae_artifact_retention_scheduled_worker_postgres_smoke({})
...
def test_ae_artifact_retention_scheduled_worker_postgres_smoke_rejects_non_test_profile() -> None:
    evidence = smoke.run_ae_artifact_retention_scheduled_worker_postgres_smoke(
...
def test_ae_artifact_retention_scheduled_worker_postgres_smoke_reports_missing_database_url() -> None:
    evidence = smoke.run_ae_artifact_retention_scheduled_worker_postgres_smoke(
        {smoke.SMOKE_ENV: "1"}
    )
...
def test_ae_artifact_retention_scheduled_worker_postgres_smoke_reports_migration_failure(
    ...
    assert evidence["status"] == "FAIL"
    assert evidence["failure_code"] == "configuration_invalid"
    assert "secret-0495" not in evidence["detail"]


def test_ae_artifact_retention_scheduled_worker_postgres_smoke_passes_with_sqlite_harness(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: SimpleNamespace(
            planned=("0112_service_worker_heartbeat_foundation",),
            applied=(),
            skipped=("0112_service_worker_heartbeat_foundation",),
        ),
    )
    ...
    assert evidence["job"]["enqueue_status"] == "ENQUEUED"
    assert evidence["job"]["duplicate_job_id"] == evidence["job"]["job_id"]
    assert evidence["job"]["queued_status"] == "QUEUED"
    assert evidence["job"]["completed_status"] == "SUCCEEDED"
    assert evidence["job"]["attempt_count"] == 1
    assert evidence["worker"]["runner_status"] == "SUCCEEDED"
    assert evidence["worker"]["handler_status"] == "SUCCEEDED"
    assert evidence["worker"]["heartbeat_status"] == "IDLE"
    ...
    assert "secret-0495" not in serialized
    assert "/data/nex-platform" not in serialized
    assert "storage_ref" not in serialized


def test_ae_artifact_retention_scheduled_worker_postgres_smoke_reports_failed_checks(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    engine = sqlite_artifact_session_factory().kw["bind"]
    monkeypatch.setattr(smoke, "build_engine", lambda _database_url: engine)
    ...
    with pytest.raises(RuntimeError, match="storage_files_retained"):
        smoke._execute_ae_artifact_retention_scheduled_worker_smoke(
            database_url=smoke_env()["NEX_AE_TEST_DATABASE_URL"],
            database_env="NEX_AE_TEST_DATABASE_URL",
        )


def test_ae_artifact_retention_scheduled_worker_postgres_smoke_wraps_execute_value_error(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    engine = sqlite_artifact_session_factory().kw["bind"]
    monkeypatch.setattr(smoke, "build_engine", lambda _database_url: engine)
...
def test_ae_artifact_retention_scheduled_worker_postgres_smoke_reports_execution_exception(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        smoke,
    ...
    monkeypatch.setattr(
        smoke,
        "_execute_ae_artifact_retention_scheduled_worker_smoke",
...
def test_ae_artifact_retention_scheduled_worker_postgres_smoke_helpers_and_redaction(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    env = smoke_env()
    env["NEX_AE_ARTIFACT_STORAGE_ROOT"] = "/data/nex-platform/private"
    clock = smoke._clock_from_sequence(("2026-09-01T00:00:00Z",))

    assert clock() == "2026-09-01T00:00:00Z"
    assert clock() == "2026-09-01T00:00:00Z"
    assert smoke._json_value(None, {"empty": True}) == {"empty": True}
    assert smoke._json_value('{"ok": true}', {}) == {"ok": True}
    assert smoke._datetime_value_or_none(None) is None
    ...
    assert "secret-0495" not in smoke._safe_detail(
    ...
    with pytest.raises(ValueError, match="database password"):
        smoke.assert_smoke_evidence_redacted("password=secret-0495", env)
    ...
    monkeypatch.setattr(
        smoke,
        "run_ae_artifact_retention_scheduled_worker_postgres_smoke",
        lambda: {
            "smoke_schema_version": smoke.SCHEMA_VERSION,
            "status": "SKIPPED",
