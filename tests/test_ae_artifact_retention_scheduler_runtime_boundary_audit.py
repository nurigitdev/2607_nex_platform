...
import run_ae_artifact_retention_scheduler_runtime_boundary_audit as audit


def protected_env() -> dict[str, str]:
    return {
        "NEX_AE_TEST_DATABASE_URL": (
            "postgresql+psycopg://nex_ae_user:secret-0491@127.0.0.1:5432/nex_ae_test"
        ),
        "NEX_AE_DATABASE_URL": (
            "postgresql+psycopg://nex_ae_user:secret-0491@127.0.0.1:5432/nex_ae_dev"
        ),
        "NEX_AE_ARTIFACT_STORAGE_ROOT": "/data/nex-platform/ae/artifacts",
        "NEX_AG_AE_ARTIFACT_SERVICE_TOKEN": "service-token-0491",
        "NEX_SERVICE_TOKEN": "service-token-shared-0491",
    }


def test_artifact_retention_scheduler_runtime_boundary_audit_passes_current_repo() -> None:
    evidence = audit.run_ae_artifact_retention_scheduler_runtime_boundary_audit({})

    assert evidence["status"] == "PASS"
    assert evidence["audit_schema_version"] == audit.SCHEMA_VERSION
    assert evidence["slice"] == "0491"
    assert evidence["decisions"]["job_type"] == audit.S50_JOB_TYPE
    assert evidence["decisions"]["worker_type"] == audit.S50_WORKER_TYPE
    assert evidence["decisions"]["default_execution_mode"] == "DRY_RUN"
    assert evidence["decisions"]["scheduler_daemon_enabled_in_s50"] is False
    assert evidence["decisions"]["physical_delete_automation_enabled_in_s50"] is False
    assert evidence["checks"]["scheduler_daemon_deferred"] is True
    assert evidence["checks"]["default_execution_mode_dry_run"] is True
    assert evidence["checks"]["physical_delete_automation_deferred"] is True
    ...
    assert "next=Slice_0492" in audit.summary_line(evidence)


def test_artifact_retention_scheduler_runtime_boundary_audit_redacts_protected_values() -> None:
    env = protected_env()

    evidence = audit.run_ae_artifact_retention_scheduler_runtime_boundary_audit(env)
    serialized = json.dumps(evidence, ensure_ascii=False)

    assert evidence["status"] == "PASS"
    assert "secret-0491" not in serialized
    assert "/data/nex-platform" not in serialized
    assert "service-token-0491" not in serialized
...
def test_artifact_retention_scheduler_runtime_boundary_audit_reports_missing_boundaries(
...
def test_artifact_retention_scheduler_runtime_boundary_audit_reports_token_failure(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    ...
    assert evidence["checks"]["shared_jobqueue_runtime_present"] is False
    assert evidence["issues"] == [
        {
            "category": "source_token_missing",
            "group": "shared_jobqueue_runtime",
...
def test_artifact_retention_scheduler_runtime_boundary_audit_helpers_output_and_main(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    ...
    assert (
        "ae_artifact_retention_scheduler_runtime_boundary_audit=pass"
...
def test_artifact_retention_scheduler_runtime_boundary_audit_cli_fail_status(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    monkeypatch.setattr(
        audit,
        "run_ae_artifact_retention_scheduler_runtime_boundary_audit",
        lambda *_: {
    ...
    assert "ae_artifact_retention_scheduler_runtime_boundary_audit=fail" in output
    assert "failing_checks=required_paths_present" in output


def test_artifact_retention_scheduler_runtime_boundary_audit_quality_gate_docs_and_readme_wired() -> None:
    ...
    assert "run_ae_artifact_retention_scheduler_runtime_boundary_audit.py" in quality_gate
    assert "Slice 0491" in docs_index
    assert "Slice 0491 starts S50" in ae_api_readme
    assert "Slice 0491 starts S50" in ag_readme
