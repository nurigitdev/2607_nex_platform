...
import run_ae_artifact_lifecycle_boundary_audit as audit


def protected_env() -> dict[str, str]:
    return {
        "NEX_AE_TEST_DATABASE_URL": (
            "postgresql+psycopg://nex_ae_user:secret-0451@127.0.0.1:5432/nex_ae_test"
        ),
        "NEX_AE_DATABASE_URL": (
            "postgresql+psycopg://nex_ae_user:secret-0451@127.0.0.1:5432/nex_ae_dev"
        ),
        "NEX_AE_ARTIFACT_STORAGE_ROOT": "/data/nex-platform/ae/artifacts",
        "NEX_AG_AE_ARTIFACT_SERVICE_TOKEN": "service-token-0451",
    }


def test_artifact_lifecycle_boundary_audit_passes_current_repo() -> None:
    evidence = audit.run_ae_artifact_lifecycle_boundary_audit({})

    assert evidence["status"] == "PASS"
    assert evidence["audit_schema_version"] == audit.SCHEMA_VERSION
    assert evidence["decisions"]["allowed_lifecycle_actions"] == [
        "ARCHIVE",
        "RESTORE",
        "MARK_DELETED",
    ]
    assert evidence["decisions"]["physical_delete_policy"] == (
        "deferred_to_retention_or_purge_track"
    )
    assert evidence["decisions"]["storage_mutation_policy"] == (
        "no_storage_payload_or_file_removal_in_s46"
    )
    ...
    assert evidence["checks"]["allowed_actions_reversible"] is True
    assert evidence["checks"]["physical_delete_deferred"] is True
    assert audit.summary_line(evidence).startswith(
        "ae_artifact_lifecycle_boundary_audit=pass "
    )
    assert "next=Slice_0452" in audit.summary_line(evidence)


def test_artifact_lifecycle_boundary_audit_redacts_protected_values() -> None:
    env = protected_env()

    evidence = audit.run_ae_artifact_lifecycle_boundary_audit(env)
    serialized = json.dumps(evidence, ensure_ascii=False)

    assert evidence["status"] == "PASS"
    assert "/data/nex-platform" not in serialized
    assert "secret-0451" not in serialized
    assert "service-token-0451" not in serialized
    assert env["NEX_AE_TEST_DATABASE_URL"] not in serialized


def test_artifact_lifecycle_boundary_audit_reports_missing_boundaries(
    ...
    evidence = audit.run_ae_artifact_lifecycle_boundary_audit({}, root_dir=tmp_path)
...
def test_artifact_lifecycle_boundary_audit_helpers_output_and_main(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
...
def test_artifact_lifecycle_boundary_audit_cli_fail_status(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    monkeypatch.setattr(
        audit,
        "run_ae_artifact_lifecycle_boundary_audit",
        lambda *_: {
            "audit_schema_version": audit.SCHEMA_VERSION,
            "status": "FAIL",
            "paths": [{"present": False}],
            "source_tokens": [{"present": False}],
            "planned_gaps": [{"already_present": False}],
    ...
    assert "ae_artifact_lifecycle_boundary_audit=fail" in output
    assert "failing_checks=required_paths_present" in output


def test_artifact_lifecycle_boundary_audit_quality_gate_docs_and_readmes_wired() -> None:
    ...
    slice_doc = (
        root / "docs" / "slices" / "0451_ae_artifact_lifecycle_boundary_audit.md"
    )

    assert "run_ae_artifact_lifecycle_boundary_audit.py --summary" in quality_gate
    assert "0451_ae_artifact_lifecycle_boundary_audit.md" in docs_index
    assert "Slice 0451" in ae_api_readme
    assert "Slice 0451" in ae_web_readme
    assert "Slice 0451" in ag_readme
