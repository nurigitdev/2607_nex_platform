...
import run_s41_artifact_runtime_closure as closure


ROOT = Path(__file__).resolve().parents[1]


def test_s41_artifact_runtime_closure_passes_for_repo() -> None:
    evidence = closure.run_s41_artifact_runtime_closure(ROOT)
...
def test_s41_artifact_runtime_closure_reports_missing_files(tmp_path: Path) -> None:
    (tmp_path / "docs" / "slices").mkdir(parents=True)

    evidence = closure.run_s41_artifact_runtime_closure(tmp_path)
...
def test_s41_artifact_runtime_closure_reports_token_failures(tmp_path: Path) -> None:
    ...
    target.write_text(
        target.read_text(encoding="utf-8").replace(
            ...
            "assert_artifact_projection_disabled",
        ),
        encoding="utf-8",
    )
    ...
    assert failed == [
        {
            "check_id": "ag_artifact_redaction_guard",
...
def test_s41_artifact_runtime_closure_cli_summary_and_json(monkeypatch, capsys) -> None:
    pass_evidence = {
    ...
    monkeypatch.setattr(
        closure,
        "run_s41_artifact_runtime_closure",
        lambda: pass_evidence,
    )

    assert closure.main(["--summary"]) == 0
    assert "s41_artifact_runtime_closure=pass" in capsys.readouterr().out
...
def test_s41_artifact_runtime_closure_read_text_missing(tmp_path: Path) -> None:
