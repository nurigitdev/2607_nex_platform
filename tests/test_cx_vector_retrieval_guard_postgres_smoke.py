...
SCRIPT = ROOT / "scripts" / "smoke" / "run_cx_vector_retrieval_guard_postgres_smoke.py"


def _module():
    spec = importlib.util.spec_from_file_location("cx_vector_retrieval_guard_smoke", SCRIPT)
...
def test_guard_profile_target_and_process_environment(monkeypatch):
    module = _module()
    assert module.run_cx_vector_retrieval_guard_postgres_smoke({})["status"] == "SKIPPED"
    monkeypatch.delenv(module.SMOKE_ENV, raising=False)
    assert module.run_cx_vector_retrieval_guard_postgres_smoke()["status"] == "SKIPPED"
    profile = module.run_cx_vector_retrieval_guard_postgres_smoke(
    ...
    target = module.run_cx_vector_retrieval_guard_postgres_smoke(
        {module.SMOKE_ENV: "1"}
    )
...
def test_success_failure_exception_summary_and_main(monkeypatch, capsys):
    ...
    passing = module.run_cx_vector_retrieval_guard_postgres_smoke(
        {module.SMOKE_ENV: "1"}
    )
    ...
    monkeypatch.setattr(
        module,
        "_execute_smoke",
        lambda *_a, **_k: {"failed_checks": ["freshness"]},
    )
    assert (
        module.run_cx_vector_retrieval_guard_postgres_smoke(
            {module.SMOKE_ENV: "1"}
        )["error"]["code"]
    ...
    monkeypatch.setattr(
        module,
        "run_cx_vector_retrieval_guard_postgres_smoke",
