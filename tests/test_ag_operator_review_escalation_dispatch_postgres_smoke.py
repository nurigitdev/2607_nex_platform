...
import run_ag_operator_review_escalation_dispatch_postgres_smoke as smoke
...
class FakeCleanupConnection:
    ...
    def execute(self, statement: object, params: object | None = None) -> FakeExecuteResult:
        self.calls += 1
        return FakeExecuteResult(rowcount={1: 2, 2: 1, 3: 1, 4: 1}[self.calls])
...
def test_ag_operator_review_escalation_dispatch_postgres_smoke_skips_when_disabled() -> None:
    evidence = smoke.run_ag_operator_review_escalation_dispatch_postgres_smoke({})
...
def test_ag_operator_review_escalation_dispatch_postgres_smoke_requires_database_url() -> None:
    evidence = smoke.run_ag_operator_review_escalation_dispatch_postgres_smoke(
        {smoke.SMOKE_ENV: "1"}
    )
...
def test_ag_operator_review_escalation_dispatch_postgres_smoke_reports_migration_failure(
...
def test_ag_operator_review_escalation_dispatch_postgres_smoke_success_is_redacted(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    evidence = smoke.run_ag_operator_review_escalation_dispatch_postgres_smoke(env)
    serialized = str(evidence)

    assert evidence["status"] == "PASS"
    assert evidence["checks"]["create_new"] is True
    assert evidence["checks"]["action_started"] is True
    assert evidence["checks"]["action_replay_idempotent"] is True
    assert evidence["checks"]["dashboard_attention_count"] is True
    ...
    assert "ag-op-dispatch-create-idem" not in serialized
...
def test_ag_operator_review_escalation_dispatch_postgres_smoke_reports_failed_checks(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    fake_engine = FakeEngine()
    monkeypatch.setattr(smoke, "run_service_migrations", _migration_result)
    ...
    monkeypatch.setattr(
        smoke,
        "_db_observations",
        lambda *args, **kwargs: {**_observations(), "event_count": 1},
    )
    monkeypatch.setattr(
        smoke,
        "_cleanup_smoke_rows",
...
def test_ag_operator_review_escalation_dispatch_postgres_smoke_reports_execution_error(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    fake_engine = FakeEngine()
    monkeypatch.setattr(smoke, "run_service_migrations", _migration_result)
...
def test_ag_operator_review_escalation_dispatch_postgres_smoke_handles_engine_error(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(smoke, "run_service_migrations", _migration_result)
    monkeypatch.setattr(
...
def test_ag_operator_review_escalation_dispatch_postgres_smoke_helpers() -> None:
    ...
    assert smoke._created_dispatch_id({"dispatch_record": {"dispatch_id": "d-0710"}}) == (
        "d-0710"
    )
    ...
    assert smoke._cleanup_smoke_rows(
        ...
        "target",
        "trace",
    ) == {"cases": 1, "escalations": 1, "dispatches": 1, "events": 2}
...
def test_ag_operator_review_escalation_dispatch_postgres_smoke_main(
    monkeypatch: pytest.MonkeyPatch,
    ...
    monkeypatch.setattr(
        smoke,
        "run_ag_operator_review_escalation_dispatch_postgres_smoke",
        lambda: skipped,
    )

    assert smoke.main(["--summary"]) == 0
    assert "ag_operator_review_escalation_dispatch_postgres_smoke=skipped" in (
...
def _migration_result(*args: object, **kwargs: object) -> SimpleNamespace:
    return SimpleNamespace(
        ...
        applied=("0710_ag_operator_review_escalation_dispatch_postgres_smoke",),
        skipped=(),
    )
...
def _observations_without_tables() -> dict[str, object]:
    return {
        ...
        "dispatching_count": 1,
        "event_count": 2,
        "event_details_dispatch_count": 2,
        "action_details_count": 1,
