...
import run_ae_artifact_retention_history_boundary_audit as audit


def protected_env() -> dict[str, str]:
    return {
        "NEX_AE_TEST_DATABASE_URL": (
            "postgresql+psycopg://nex_ae_user:secret-0471@127.0.0.1:5432/nex_ae_test"
        ),
        "NEX_AE_DATABASE_URL": (
            "postgresql+psycopg://nex_ae_user:secret-0471@127.0.0.1:5432/nex_ae_dev"
        ),
        "NEX_AE_ARTIFACT_STORAGE_ROOT": "/data/nex-platform/ae/artifacts",
        "NEX_SERVICE_TOKEN": "service-token-0471",
    }


def test_artifact_retention_history_boundary_audit_passes_current_repo() -> None:
    evidence = audit.run_ae_artifact_retention_history_boundary_audit({})

    assert evidence["status"] == "PASS"
    assert evidence["audit_schema_version"] == audit.SCHEMA_VERSION
    assert evidence["decisions"]["history_table"] == "ae_artifact_retention_executions"
    assert evidence["decisions"]["execution_schema"] == (
        "ae_artifact_retention_execution.v1"
    )
    assert evidence["decisions"]["history_schema"] == (
        "ae_artifact_retention_execution_history.v1"
    )
    assert evidence["decisions"]["history_scope"] == "tenant_workspace_owner"
    assert evidence["decisions"]["storage_ref_allowed"] is False
    assert evidence["checks"]["metadata_only_history_policy"] is True
    assert evidence["checks"]["idempotency_history_policy"] is True
    ...
    assert "next=Slice_0472" in audit.summary_line(evidence)


def test_artifact_retention_history_boundary_audit_redacts_protected_values() -> None:
    env = protected_env()

    evidence = audit.run_ae_artifact_retention_history_boundary_audit(env)
    serialized = json.dumps(evidence, ensure_ascii=False)

    assert evidence["status"] == "PASS"
    assert "secret-0471" not in serialized
    assert "/data/nex-platform" not in serialized
    assert "service-token-0471" not in serialized
    assert env["NEX_AE_TEST_DATABASE_URL"] not in serialized


def test_artifact_retention_history_boundary_audit_reports_missing_boundaries(
...
def test_artifact_retention_history_boundary_audit_helpers_output_and_main(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
...
def test_artifact_retention_history_boundary_audit_cli_fail_status(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    monkeypatch.setattr(
        audit,
        "run_ae_artifact_retention_history_boundary_audit",
        lambda *_: {
    ...
    assert "ae_artifact_retention_history_boundary_audit=fail" in output
    assert "failing_checks=required_paths_present" in output


def test_artifact_retention_history_boundary_audit_quality_gate_docs_and_readme_wired() -> None:
    ...
    slice_doc = (
        ...
        / "docs"
        / "slices"
        / "0471_ae_artifact_retention_execution_history_boundary_audit.md"
    )

    assert "run_ae_artifact_retention_history_boundary_audit.py --summary" in quality_gate
    assert "0471_ae_artifact_retention_execution_history_boundary_audit.md" in docs_index
    assert "Slice 0471" in ae_api_readme
