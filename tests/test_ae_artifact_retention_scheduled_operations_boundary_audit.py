...
import run_ae_artifact_retention_scheduled_operations_boundary_audit as audit


def protected_env() -> dict[str, str]:
    return {
        "NEX_AE_TEST_DATABASE_URL": (
            "postgresql+psycopg://nex_ae_user:secret-0481@127.0.0.1:5432/nex_ae_test"
        ),
        "NEX_AE_DATABASE_URL": (
            "postgresql+psycopg://nex_ae_user:secret-0481@127.0.0.1:5432/nex_ae_dev"
        ),
        "NEX_AE_ARTIFACT_STORAGE_ROOT": "/data/nex-platform/ae/artifacts",
        "NEX_AG_AE_ARTIFACT_SERVICE_TOKEN": "service-token-0481",
        "NEX_SERVICE_TOKEN": "service-token-shared-0481",
    }


def test_artifact_retention_scheduled_operations_boundary_audit_passes_current_repo() -> None:
    evidence = audit.run_ae_artifact_retention_scheduled_operations_boundary_audit({})

    assert evidence["status"] == "PASS"
    assert evidence["audit_schema_version"] == audit.SCHEMA_VERSION
    assert evidence["slice"] == "0481"
    assert evidence["decisions"]["artifact_system_of_record"] == "nex-ae-api"
    assert evidence["decisions"]["operator_projection_owner"] == "nex-ag"
    assert evidence["decisions"]["default_batch_mode"] == "DRY_RUN"
    assert evidence["decisions"]["supported_retention_day_presets"] == [15, 30]
    assert evidence["decisions"]["scheduled_batch_timezone"] == "Asia/Seoul"
    assert evidence["decisions"]["scheduled_batch_window_local_time"] == "02:00-05:00"
    assert evidence["decisions"]["scheduled_batch_window"] == {
        "start_local_time": "02:00",
        "end_local_time": "05:00",
    }
    assert evidence["decisions"]["execute_requires_delete_enabled"] is True
    assert evidence["decisions"]["execute_requires_storage_mutation_enabled"] is True
    assert evidence["decisions"]["execute_requires_database_row_delete_enabled"] is True
    assert evidence["checks"]["batch_mutation_deferred"] is True
    assert evidence["checks"]["ag_direct_database_access_disallowed"] is True
    ...
    assert "next=Slice_0482" in audit.summary_line(evidence)


def test_artifact_retention_scheduled_operations_boundary_audit_redacts_protected_values() -> None:
    env = protected_env()

    evidence = audit.run_ae_artifact_retention_scheduled_operations_boundary_audit(env)
    serialized = json.dumps(evidence, ensure_ascii=False)

    assert evidence["status"] == "PASS"
    assert "secret-0481" not in serialized
    assert "/data/nex-platform" not in serialized
    assert "service-token-0481" not in serialized
...
def test_artifact_retention_scheduled_operations_boundary_audit_reports_missing_boundaries(
...
def test_artifact_retention_scheduled_operations_boundary_audit_reports_token_failure(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    ...
    assert evidence["checks"]["schedule_policy_baseline_present"] is False
    assert evidence["issues"] == [
        {
            "category": "source_token_missing",
            "group": "schedule_policy_baseline",
...
def test_artifact_retention_scheduled_operations_boundary_audit_helpers_output_and_main(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    ...
    assert all(item["blocking"] is False for item in audit.build_gap_results())

    assert audit.main(["--summary", "--output", str(output_path)]) == 0
    assert (
        "ae_artifact_retention_scheduled_operations_boundary_audit=pass"
...
def test_artifact_retention_scheduled_operations_boundary_audit_cli_fail_status(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    monkeypatch.setattr(
        audit,
        "run_ae_artifact_retention_scheduled_operations_boundary_audit",
        lambda *_: {
    ...
    assert "ae_artifact_retention_scheduled_operations_boundary_audit=fail" in output
    assert "failing_checks=required_paths_present" in output


def test_artifact_retention_scheduled_operations_boundary_audit_quality_gate_docs_and_readme_wired() -> None:
    ...
    assert (
        "run_ae_artifact_retention_scheduled_operations_boundary_audit.py --summary"
        in quality_gate
    )
    assert "0481_ae_artifact_retention_scheduled_operations_boundary_audit.md" in docs_index
    assert "Slice 0481" in ae_api_readme
    assert "Slice 0481" in ag_readme
