...
import run_ag_recovery_notification_delivery_postgres_smoke as smoke
...
def _context() -> dict[str, str]:
    return {
        "case_id": "case-0848-unit",
        "target_id": "target-0848-unit",
        "request_id": "request-0848-unit",
        "trace_id": "4bf92f3577b34da6a3ce929d0e0e4736",
        "worker_id": "worker-0848-unit",
        "case_idempotency_key": "case-idem-0848-unit",
        "escalation_idempotency_key": "escalation-idem-0848-unit",
        "dispatch_idempotency_key": "dispatch-idem-0848-unit",
    }


def test_smoke_skips_without_opt_in() -> None:
    evidence = smoke.run_ag_recovery_notification_delivery_postgres_smoke({})
...
def test_smoke_redacts_migration_failure(
    ...
    evidence = smoke.run_ag_recovery_notification_delivery_postgres_smoke(env)
...
def test_smoke_orchestrates_result_and_cleanup(
    ...
    evidence = smoke.run_ag_recovery_notification_delivery_postgres_smoke(_env())
...
def test_execute_smoke_persists_and_executes_targeted_delivery(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    engine = FakeEngine()
    ...
    assert evidence["execution"]["execution_status"] == "COMPLETED"
    assert evidence["execution"]["provider_mode"] == "mock_first_only"
    dispatch_id = evidence["owned_context"]["dispatch_id"]
    assert dispatch_store.get(dispatch_id)["dispatch_status"] == "SUCCEEDED"


def test_cleanup_owned_rows_and_helper_contracts() -> None:
    class Result:
    ...
    cleanup = smoke._cleanup_owned_rows(Engine(), context=_context())
...
def test_database_observation_reads_postgresql_rows() -> None:
    class Result:
    ...
    assert observation["succeeded_count"] == 1
    assert observation["execution_metadata_count"] == 1


def test_cleanup_failure_and_evidence_redaction() -> None:
    class FailingEngine(FakeEngine):
        def begin(self):
            raise AttributeError("no transaction")
    ...
    with pytest.raises(ValueError, match="contains a secret"):
        smoke.assert_smoke_evidence_redacted(
            f"evidence {smoke.RAW_PROVIDER_TOKEN}",
            _env(),
        )
...
def test_main_prints_summary_and_json(
    ...
    monkeypatch.setattr(
        smoke,
        "run_ag_recovery_notification_delivery_postgres_smoke",
