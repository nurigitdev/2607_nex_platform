...
import run_ae_artifact_library_management_boundary_audit as audit


def protected_env() -> dict[str, str]:
    return {
        "NEX_AE_TEST_DATABASE_URL": (
            "postgresql+psycopg://nex_ae_user:secret-0441@127.0.0.1:5432/nex_ae_test"
        ),
        "NEX_AE_DATABASE_URL": (
            "postgresql+psycopg://nex_ae_user:secret-0441@127.0.0.1:5432/nex_ae_dev"
        ),
        "NEX_AE_ARTIFACT_STORAGE_ROOT": "/data/nex-platform/ae/artifacts",
        "NEX_AG_AE_ARTIFACT_SERVICE_TOKEN": "service-token-0441",
    }


def test_artifact_library_management_boundary_audit_passes_current_repo() -> None:
    evidence = audit.run_ae_artifact_library_management_boundary_audit({})
    ...
    assert audit.summary_line(evidence).startswith(
        "ae_artifact_library_management_boundary_audit=pass "
    )
    assert "next=Slice_0442" in audit.summary_line(evidence)


def test_artifact_library_management_boundary_audit_redacts_protected_values() -> None:
    env = protected_env()

    evidence = audit.run_ae_artifact_library_management_boundary_audit(env)
    serialized = json.dumps(evidence, ensure_ascii=False)

    assert evidence["status"] == "PASS"
    assert "/data/nex-platform" not in serialized
    assert "secret-0441" not in serialized
    assert "service-token-0441" not in serialized
    assert env["NEX_AE_TEST_DATABASE_URL"] not in serialized
    assert env["NEX_AE_ARTIFACT_STORAGE_ROOT"] not in serialized


def test_artifact_library_management_boundary_audit_reports_missing_boundaries(
    ...
    assert evidence["checks"]["api_metadata_boundary_present"] is False
...
def test_artifact_library_management_boundary_audit_helpers_output_and_main(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
...
def test_artifact_library_management_boundary_audit_cli_fail_status(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    monkeypatch.setattr(
        audit,
        "run_ae_artifact_library_management_boundary_audit",
        lambda *_: {
    ...
    assert "ae_artifact_library_management_boundary_audit=fail" in output
    assert "failing_checks=required_paths_present" in output


def test_artifact_library_management_boundary_audit_quality_gate_docs_and_readmes_wired() -> None:
    ...
    slice_doc = (
        ...
        / "docs"
        / "slices"
        / "0441_ae_artifact_library_management_boundary_audit.md"
    )

    assert "run_ae_artifact_library_management_boundary_audit.py --summary" in quality_gate
    assert "0441_ae_artifact_library_management_boundary_audit.md" in docs_index
    assert "Slice 0441" in ae_api_readme
    assert "Slice 0441" in ae_web_readme
