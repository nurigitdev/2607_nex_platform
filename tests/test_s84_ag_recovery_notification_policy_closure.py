...
import run_s84_ag_recovery_notification_policy_closure as closure
...
def test_s84_recovery_notification_policy_closure_passes_repo() -> None:
    evidence = closure.run_s84_ag_recovery_notification_policy_closure()

    assert evidence["status"] == "PASS"
    assert evidence["closure_schema_version"] == closure.SCHEMA_VERSION
    assert evidence["slice_range"] == "0831-0840"
    assert evidence["boundary"] == "ag_recovery_notification_policy_preview_only"
    ...
    assert evidence["external_delivery_implemented"] is False
    assert len(evidence["closure_surfaces"]) == 9
    assert evidence["runtime"]["preview_plan_status"] == "PREVIEW_ONLY"
    assert evidence["runtime"]["delivery_ready_plan_status"] == "READY"
    assert evidence["runtime"]["delivery_performed"] is False
...
def test_s84_recovery_notification_policy_closure_reports_missing_repo(
    tmp_path: Path,
) -> None:
    evidence = closure.run_s84_ag_recovery_notification_policy_closure(tmp_path)
...
def test_s84_recovery_notification_policy_closure_reports_token_failure(
    tmp_path: Path,
...
def test_s84_recovery_notification_policy_closure_reports_privacy_failure(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        closure,
...
def test_s84_recovery_notification_policy_closure_reports_privacy_exception(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        closure,
...
def test_s84_recovery_notification_policy_closure_helpers(tmp_path: Path) -> None:
    smoke_doc = tmp_path / closure.POSTGRES_SMOKE_DOC
    smoke_doc.parent.mkdir(parents=True)
    smoke_doc.write_text(
        "Current migrations ran\n"
        "ag_recovery_notification_postgres_smoke=pass "
        "database=nex_ag_test backend=postgresql plan=SUPPRESSED "
        "dispatches=0 events=0 cleaned=1\n"
    ...
    assert runtime["default_policy"]["policy_status"] == "ENABLED"
    assert runtime["operations_status"] == "READY"
    assert runtime["unavailable_status"] == "SOURCE_UNAVAILABLE"
    assert closure._recovery_plan()["projection_status"] == "READY"


def test_s84_recovery_notification_policy_closure_main(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    monkeypatch.setattr(
        closure,
        "run_s84_ag_recovery_notification_policy_closure",
        lambda: {
            "status": "PASS",
            "slice_range": closure.SLICE_RANGE,
            "runtime": {"delivery_performed": False},
            "checks": {
